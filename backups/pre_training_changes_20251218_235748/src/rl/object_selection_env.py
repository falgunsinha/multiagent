...
class ObjectSelectionEnv(gym.Env):
    ...
    def action_masks(self) -> np.ndarray:
        ...
        Now uses robust 3-layer reachability check with fallback.
        ...
        # Last resort: If still no valid actions, allow all unpicked cubes
        # This prevents deadlock in edge cases
        if not mask.any():
            for i in range(self.total_objects):
                if i not in self.objects_picked:
                    mask[i] = True
    ...
    def _generate_random_obstacles(self, grid_size: int, occupied_cells: set) -> list:
        ...
        # Generate 1-3 random obstacles (same logic as visualizers)
        max_obstacles = max(0, min(3, available_cells))  # Cap at 3, or 0 if no room
        min_obstacles = 1 if max_obstacles > 0 else 0    # At least 1 if room exists
    ...
    def _calculate_obstacle_score_with_unpicked_cubes(self, position: np.ndarray, target_action: int) -> float:
        ...
        # Check distance to random obstacles
        for obs_pos in self.random_obstacle_positions:
        ...
        for i in range(self.total_objects):
            ...
            cube_pos = self.object_positions[i]
            distance = np.linalg.norm(position[:2] - cube_pos[:2])
            min_distance = min(min_distance, distance)
