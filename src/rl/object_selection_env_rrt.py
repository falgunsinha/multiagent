...
class ObjectSelectionEnvRRT(ObjectSelectionEnv):
    ...
    def __init__(
        ...
        # Use DDQN training spacing for Isaac Sim RRT
        ...
        super().__init__(
            ...
            cube_spacing=ddqn_spacing  # Override with DDQN spacing
        )

        self.execute_picks = execute_picks

        # Initialize Isaac Sim RRT path estimator (for training)
        # UPDATED: Cell size matches DDQN training spacing
        self.rrt_estimator = IsaacSimRRTPathEstimator(
            grid_size=training_grid_size,
            cell_size=0.13 if training_grid_size > 3 else 0.15,  # Match DDQN training spacing
    ...
    def _is_reachable(self, obj_idx: int) -> bool:
        ...
        # UPDATED: Use RRT estimator if franka_controller is not available
        # This allows testing without a full controller
        if self.franka_controller is not None:
            ...
            # Fallback: Use RRT estimator for reachability check
            obj_position = self.object_positions[obj_idx]

            # Update RRT grid excluding target cube
            self._update_rrt_grid(target_cube_idx=obj_idx)

            # Use RRT estimator's check_reachability method
            # This is faster than full path planning
            return self.rrt_estimator.check_reachability(
                self.ee_position,
                obj_position
            )
        else:
            # Last resort: Use distance-based check
    ...
    def _generate_random_objects(self, use_pygame_style=True):
        ...
        if use_pygame_style:
            ...
            # Convert occupied_cells list to set of tuples for base class
