...
class AStarPathEstimator:
    ...
    def __init__(self, grid_size: int = 6, cell_size: float = 0.13):
        """
        Initialize A* path estimator.

        Args:
            grid_size: Size of the grid (e.g., 6 for 6x6)
            cell_size: Size of each grid cell in meters (resolution)
        """
        self.grid_size = grid_size
        self.cell_size = cell_size  # resolution
...
class RRTPathEstimator:
    ...
    def __init__(self, grid_size: int = 6, cell_size: float = 0.13, franka_controller=None):
        """
        ...
        # RRT parameters (tuned for 6x6 grid)
        self.expand_dis = 1.0  # Expansion distance in grid cells (reduced for better navigation)
        self.path_resolution = 0.5  # Path resolution for steering
        self.max_iter = 5000  # Maximum iterations (increased for better success rate)
        self.goal_sample_rate = 25  # Goal sampling rate (%) - increased for faster convergence
        self.robot_radius = 0.0  # Robot radius for collision checking
    ...
    def update_occupancy_grid(self, object_positions: List[np.ndarray], obstacle_positions: List[np.ndarray]):
        ...
        # Update grid map - use obstacle size that blocks the cell and immediate vicinity
        # Size 0.51 means obstacle radius is 0.51 grid cells (slightly > 0.5 for safety margin)
        # With robot_radius=0.0, collision threshold = 0.51
        # With path_resolution=0.5, intermediate points at distance 0.5 will collide (0.5 < 0.51)
        # Adjacent cell centers at distance 1.0 are still free (1.0 > 0.51)
        for obj_pos in object_positions:
            grid_x, grid_y = self._world_to_grid(obj_pos[:2])
            if 0 <= grid_x < self.grid_size and 0 <= grid_y < self.grid_size:
                self.obstacle_map[grid_y, grid_x] = True
                # Add to obstacle list for RRT (x, y, size) - size 0.51 for safety margin
            ...
            if self.check_collision(new_node, self.obstacle_list, self.robot_radius):
                ...
                if self.calc_dist_to_goal(new_node, end) <= self.expand_dis:
                    ...
                    if self.check_collision(final_node, self.obstacle_list, self.robot_radius):
                        ...
                        return self.generate_final_course(len(node_list) - 1, node_list)
