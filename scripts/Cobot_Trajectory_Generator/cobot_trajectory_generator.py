...
class CobotTrajectoryGeneration():
    ...
    def setup_cspace_trajectory(self):
        ...
        # Delete old visualization frames first
...
def on_reset_scene_clicked():
    ...
    # Clear World instance first (this removes most prims)
    if g_world is not None:
        World.clear_instance()
        g_world = None

    # Delete any remaining prims from the stage
    ...
    if stage:
        ...
        # Delete visualization frames if they exist
        if get_prim_at_path("/visualized_frames"):
            delete_prim("/visualized_frames")

        # Delete robot if it exists outside /World
