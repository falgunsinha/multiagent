...
class FrankaRRTDynamicGrid:
    ...
    async def _pick_and_place_cube(self, cube, cube_name, is_last_cube=False):
        ...
        try:
            ...
            # Increased to 10cm to prevent gripper from hitting previously placed cubes
            place_spacing = 0.10  # 10cm spacing in container
            ...
            for _ in range(60):  # Increased from 50 to 60 frames
                await omni.kit.app.get_app().next_update_async()

            # Step 3: Descend to pick position using IK (straight down motion)
            print(f"Step 3: Descending to pick: {pick_pos}")
            # Use IK for straight vertical descent to avoid lateral movement
            success = await self._move_to_target_ik(pick_pos, orientation)
            if not success:
                print("  IK descent failed, trying RRT...")
            ...
            for _ in range(30):  # Increased from 20 to 30 frames
                await omni.kit.app.get_app().next_update_async()
            ...
            # Step 7: Move to high position above container (to avoid hitting container walls)
            # This ensures we approach from above, not from the side
            place_high = place_pos + np.array([0.0, 0.0, 0.25])  # 25cm above place position
            print(f"Step 7: Moving to high position above place: {place_high}")
            plan = self._plan_to_target(place_high, orientation)
            if plan is None:
                print("Failed to plan to high position above place")
                return False
            await self._execute_plan(plan)

            # Step 8: Descend to place approach (15cm above final position)
            print(f"Step 8: Descending to place approach: {place_approach}")
            ...
            # Step 9: Descend to place position using IK (straight down motion)
            print(f"Step 9: Descending to place: {place_pos}")
            # Use IK for straight vertical descent to avoid lateral movement
            ...
            # Step 10: Open gripper (release cube)
            print("Step 10: Opening gripper to release cube")
            articulation_controller = self.franka.get_articulation_controller()
            open_action = ArticulationAction(
                joint_positions=self.gripper.joint_opened_positions,
                joint_indices=np.array([7, 8])
            ...
            # Step 11: Move up slightly (5cm) to clear the placed cube before ascending
            # This prevents gripper from hitting the cube when moving to next position
            place_clearance = place_pos + np.array([0.0, 0.0, 0.05])  # 5cm above place position
            print(f"Step 11: Moving up to clear cube: {place_clearance}")
            success = await self._move_to_target_ik(place_clearance, orientation, num_steps=15)
            if not success:
                print("  IK clearance move failed, trying RRT...")
            ...
            # Step 12: Ascend to high position above container
            print(f"Step 12: Ascending to high position: {place_high}")
            plan = self._plan_to_target(place_high, orientation)
            if plan is None:
                print("Failed to plan ascent to high position")
                return False
            ...
            if is_last_cube:
                ...
                # Step 13: Move to safe intermediate position (helps avoid invalid configurations)
                ...
                print(f"Step 13: Moving to safe intermediate position: {safe_intermediate}")
                try:
                ...
                # Step 14: Return to home position (safe configuration for next pick)
                print(f"Step 14: Returning to home position: {home_pos}")
                try:
                ...
                # Step 15: Close gripper to reset
                print("Step 15: Closing gripper")
                articulation_controller = self.franka.get_articulation_controller()
                close_action = ArticulationAction(
                    joint_positions=self.gripper.joint_closed_positions,
                    joint_indices=np.array([7, 8])
                ...
                # For cubes 1-3, move to safe intermediate position before next pick
                print("  → More cubes to pick, moving to safe position before next cube")

                # Step 13: Move to safe intermediate position (above container, centered)
                # This prevents abrupt movements when transitioning between cubes
                safe_intermediate = np.array([0.45, 0.0, 0.35])  # Centered, high position
                print(f"Step 13: Moving to safe intermediate position: {safe_intermediate}")
                try:
                    plan = self._plan_to_target(safe_intermediate, orientation)
                    if plan is not None:
                        ...
                        print("Warning: Could not plan to intermediate position")
                except Exception as e:
                    print(f"Warning: Error moving to intermediate position: {e}")

                # Step 14: Close gripper (prepare for next pick)
                print("Step 14: Closing gripper (prepare for next pick)")
                articulation_controller = self.franka.get_articulation_controller()
                close_action = ArticulationAction(
                    joint_positions=self.gripper.joint_closed_positions,
                    joint_indices=np.array([7, 8])
    ...
    async def _move_to_target_ik(self, target_position, target_orientation, num_steps=30):
        ...
        Args:
            ...
            num_steps: Number of interpolation steps (default: 30)
