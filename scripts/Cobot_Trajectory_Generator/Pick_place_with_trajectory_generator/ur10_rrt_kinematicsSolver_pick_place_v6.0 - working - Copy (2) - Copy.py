...
class UR10RRTKinematicsSolver:
    ...
    async def _pick_and_place_cube_hybrid(self, cube, cube_name, color_name, cube_index, is_last_cube=False):
        ...
        try:
            ...
            # Available space for grid (leave margins on each side)
            margin_left = 0.08
            margin_right = 0.10  # Increased right margin to 10cm
            margin_y = 0.08
            usable_length = container_length - (margin_left + margin_right)
            usable_width = container_width - 2 * margin_y
            ...
            # Release cube 5cm above container surface for smooth placement
            place_pos = container_pos + np.array([
                offset_x,
                offset_y,
                container_height/2 + cube_size/2 + 0.05
            ])
