...
class UR10RRTKinematicsSolver:
    ...
    async def _create_container(self, position):
        ...
        # Original: 160x120x64cm → Scaled bigger for cylinders
        # Increased scale from [0.3, 0.3, 0.2] to [0.4, 0.4, 0.25] for bigger cylinders
    ...
    def _create_cube_grid(self):
        ...
        # Cylinder parameters (bigger than cubes)
        cylinder_radius = 0.04  # 4cm radius (8cm diameter)
        cylinder_height = 0.10  # 10cm height
        ...
        # Use same color for all cylinders (white)
        cylinder_color = np.array([1.0, 1.0, 1.0])  # White color
        ...
        for row in range(self.grid_length):
            for col in range(self.grid_width):
                ...
                # Generate unique cylinder name with timestamp
                # Format: Cylinder_R{row}_C{col}_{timestamp}
                timestamp = int(time.time() * 1000)
                cube_name = f"Cylinder_R{row+1}_C{col+1}_{timestamp}"
                cube_display_name = f"Cylinder_{row+1}_{col+1}"  # For logging
                ...
                print(f"  {cube_display_name} (White) created as: {cube_name}")
    ...
    async def _pick_and_place_cube_hybrid(self, cube, cube_name, color_name, cube_index, is_last_cube=False):
        ...
        try:
            ...
            print(f"[PICK] Cylinder position: {cube_pos}")

            # Calculate place position in container using actual dimensions
            # Cylinder parameters (must match creation parameters)
            cylinder_radius = 0.04  # 4cm radius
            cylinder_height = 0.10  # 10cm height
            container_pos, _ = self.container.get_world_pose()

            # Use actual container dimensions (stored during creation)
            container_length = self.container_dimensions[0]  # X-axis (0.64m)
            container_width = self.container_dimensions[1]   # Y-axis (0.48m)
            container_height = self.container_dimensions[2]  # Z-axis (0.16m)

            # Get total number of cylinders from UI
            ...
            # PLACEMENT STRATEGY:
            # Distribute cylinders evenly across the FULL container area
            # Use available space after margins to calculate optimal spacing
            # This ensures equal distribution across the entire container

            # Grid layout (NOTE: grid_length = rows along Y, grid_width = cols along X)
            ...
            # Margin from container edges (20cm from each edge for safety)
            margin = 0.20

            # Calculate AVAILABLE space for placement (after margins)
            available_length = container_length - 2 * margin  # X-axis available space
            available_width = container_width - 2 * margin    # Y-axis available space

            # Calculate spacing to distribute cylinders evenly across FULL available area
            # This ensures equal distribution using the entire container space
            if cubes_per_row > 1:
            ...
            # This centers the grid and distributes cylinders evenly
            if cubes_per_row > 1:
            ...
            # Release cylinder 3cm above container surface for smooth placement
            ...
            if cube_index == 0:
                print(f"[PLACE] Cylinder 1 at CENTER: {place_pos}")
            else:
                print(f"[PLACE] Cylinder {cube_index+1}: {place_pos} | Grid R{row+1}C{col+1} | Spacing: X={spacing_x:.3f}m Y={spacing_y:.3f}m")

            # Calculate key positions
            cylinder_half_height = cylinder_height / 2.0

            # Pre-pick position: 12cm above cylinder
            pre_pick_pos = cube_pos + np.array([0.0, 0.0, 0.12])

            # Pick position: 3mm above cylinder top (for suction - closer contact)
            pick_height = cube_pos[2] + cylinder_half_height + 0.003
            pick_pos = np.array([cube_pos[0], cube_pos[1], pick_height])

            # Pre-place position: 25cm above place location (higher to avoid hitting container)
            ...
            # Increased waypoints from 20 to 30 for smoother approach
            ...
            current_ee_pos, _ = self.ur10.end_effector.get_world_pose()
            retreat_pos = current_ee_pos + np.array([0.0, 0.0, 0.08])  # Reduced from 0.12 to 0.08
            # Increased waypoints from 20 to 30 for smoother retreat
            ...
            # Add settling time after retreat to prevent jerky motion
            for _ in range(15):
                await omni.kit.app.get_app().next_update_async()
            ...
            current_ee_pos, _ = self.ur10.end_effector.get_world_pose()
            retreat_pos = current_ee_pos + np.array([0.0, 0.0, 0.10])  # Reduced from 0.18 to 0.10
            # Increased waypoints from 20 to 30 for smoother retreat
            success = await self._taskspace_straight_line(current_ee_pos, retreat_pos, orientation, num_waypoints=30)
            if not success:
