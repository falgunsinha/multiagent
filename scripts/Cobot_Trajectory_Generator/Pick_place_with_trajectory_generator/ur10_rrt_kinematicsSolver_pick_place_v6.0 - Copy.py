...
class UR10HybridPickPlace:
    """UR10 Pick and Place with RRT + Kinematics Solver (Franka Pattern)"""
    ...
    def build_ui(self):
        ...
        with self.window.frame:
            with ui.VStack(spacing=10):
                ...
                # Info
                ui.Spacer(height=5)
                with ui.CollapsableFrame("Hybrid Approach Info", collapsed=True):
                    with ui.VStack(spacing=3):
                        ui.Label("• Pick Approach: Task-Space (straight down)", style={"font_size": 11})
                        ui.Label("• Pick Retreat: Task-Space (straight up)", style={"font_size": 11})
                        ui.Label("• Travel: C-Space + RRT (efficient + safe)", style={"font_size": 11})
                        ui.Label("• Place Approach: Task-Space (straight down)", style={"font_size": 11})
                        ui.Label("• Place Retreat: Task-Space (straight up)", style={"font_size": 11})
    ...
    async def _load_scene(self):
        ...
        try:
            ...
            if gripper_prim.IsValid():
                ...
                print(f"  Using default gripper configuration")
    ...
    async def _pick_and_place_cube_hybrid(self, cube, cube_name, color_name, cube_index, is_last_cube=False):
        ...
        try:
            ...
            success = await self._taskspace_straight_line(current_ee_pos, pick_pos, orientation)
            if not success:
            ...
            success = await self._taskspace_straight_line(current_ee_pos, retreat_pos, orientation)
            if not success:
            ...
            current_ee_pos, _ = self.ur10.end_effector.get_world_pose()
            success = await self._taskspace_straight_line(current_ee_pos, place_pos, orientation)
            if not success:
    ...
    async def _execute_plan(self, plan):
        ...
        for i, action in enumerate(plan):
            ...
            # Add small delay every 5 steps for smoother motion
            if i % 5 == 0:
                await omni.kit.app.get_app().next_update_async()
...
if __name__ == "__main__":
    app = UR10HybridPickPlace()
    print("="*60)
    print("PICK AND PLACE WITH TRAJECTORY GENERATION")
    print("="*60)
    print("Approach: Task-Space + C-Space + RRT")
    print("  • Pick Approach/Retreat: Task-Space (straight-line)")
    print("  • Travel: C-Space + RRT (efficient + safe)")
    print("  • Place Approach/Retreat: Task-Space (straight-line)")
    print("="*60)
