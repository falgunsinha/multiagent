...
async def setup_scene():
    ...
    container_name = f"container_{timestamp}"
    franka_prim_path = f"/World/Franka_{timestamp}"
    cube_prim_path = f"/World/Cube_{timestamp}"
    container_prim_path = f"/World/Container_{timestamp}"
    ...
    # Create SingleXFormPrim wrapper without setting transform in constructor
    container_xform = SingleXFormPrim(prim_path=container_prim_path, name=container_name)

    # Set scale and pose using methods to avoid extra attributes
    container_xform.set_local_scale(np.array([0.3, 0.3, 0.3]))
    container_xform.set_local_pose(translation=container_translation)
