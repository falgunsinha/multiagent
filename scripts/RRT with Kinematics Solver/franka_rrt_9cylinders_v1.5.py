...
NEW FEATURES:
- Franka robot mounted on stand
- Blue cylinders and container on table
...
class FrankaRRTDynamicGrid:
    ...
    def __init__(self):
        ...
        self.obstacle_1_acceleration = 3.0  # Acceleration magnitude in m/s² (reduced from 6.0)
        self.obstacle_1_min_x = 0.65  # Left boundary (0.65m)
        self.obstacle_1_max_x = 0.85  # Right boundary (0.85m) - 0.20m total travel (reduced range)
        ...
        self.current_grasp_name = "grasp_0"  # Default grasp to use
        self._load_grasp_configuration()

        self.build_ui()

    def _load_grasp_configuration(self):
        """Load grasp configuration from YAML file."""
        try:
    ...
    async def _load_scene(self):
        ...
        try:
            ...
            if stand_prim.IsValid():
                ...
                print("  [OK] Robot stand (textured table) configured as static collider")
            ...
            robot_translation = stand_translation  # Same as stand (0, 0, 0.6)
            ...
            print(f"  [OK] Franka USD loaded, gripper will be created after cylinders")
            ...
            # Orientation: Euler angles [0.0, 37.0, 90.0] in degrees
            position = np.array([0.9, 0.16, 1.3])  # World coordinates
            # Convert Euler angles to quaternion
            orientation = euler_angles_to_quats(np.array([0.0, 37.0, 90.0]))
            ...
            if camera_prim:
                # Set scale to 0.3 using Xformable API
                ...
                if focal_length_attr:
                    ...
                    print(f"[DEPTH CAMERA] Focal length set to 1.3mm")
                else:
            ...
            # Container position on table
            container_position = np.array([0.4, 0.45, 0.6])  # X: 0.4, Y: 0.45
            ...
            # Use SAME spacing logic as franka_rrt_2cubes_v1.5 (no scaling multiplier)
            if total_cubes <= 4:
                cylinder_spacing = 0.18
            elif total_cubes <= 9:
                cylinder_spacing = 0.15
            else:
                cylinder_spacing = 0.13

            # Use SAME grid center as franka_rrt_2cubes_v1.5 (ground-based coordinates)
            grid_center_x = 0.45
            grid_center_y = -0.10
            ...
            for row in range(self.grid_length):
                for col in range(self.grid_width):
                    ...
                    # After 1.5x scale, cylinder height will be ~7.725cm, so center at 0.64 + half_height
                    cylinder_scale = np.array([1.5, 1.5, 1.5])
                    ...
                    cylinder = self.world.scene.add(
                        DynamicCylinder(
                            ...
                            color=np.array([0.0, 0.0, 0.5]),  # Dark blue (will be overridden by emissive shader)
            ...
            print("=== Applying Emissive Shader to Shared Materials ===")
            shader_count = 0
                    ...
                    if "visual" in material_name.lower() and "material" in material_name.lower():
                        ...
                        for shader_name in ["shader", "Shader"]:
                            ...
                            if shader_prim.IsValid():
                                ...
                                # Set emissive color: #023066 (R:2, G:48, B:102 in 0-255 range)
                                emissive_color = Gf.Vec3f(0.00784, 0.18824, 0.4)
                                ...
                                shader_count += 1
                                print(f"  [OK] Applied shader to {shader_prim_path}")
                                break

            print(f"  [OK] Applied emissive shader (#023066) to {shader_count} shared materials")
            ...
            # Cylinder is ~6.45cm diameter (2.5x scale), so open to 10cm (5cm per finger) for clearance
            ...
            self.franka = self.world.scene.add(
                SingleManipulator(
                    ...
                    position=np.array([0.0, 0.0, 0.6]),  # Fixed position (0, 0, 0.6)
                    orientation=np.array([1.0, 0.0, 0.0, 0.0])
                )
            )
            print(f"  [OK] Franka positioned at (0.0, 0.0, 0.6) (on stand, no fixed joints)")

            # Initialize physics and reset AFTER adding all objects (like UR10e)
            self.world.reset()
            ...
            # Verify and re-set focal length after annotators are attached
            # Sometimes annotators can reset camera parameters
            self.depth_camera.set_focal_length(1.3)

            # Verify via USD API as well
            from omni.isaac.core.utils.stage import get_current_stage
            stage = get_current_stage()
            camera_prim = stage.GetPrimAtPath(depth_camera_prim_path)
            if camera_prim:
                focal_length_attr = camera_prim.GetAttribute("focalLength")
                if focal_length_attr:
                    current_focal = focal_length_attr.Get()
                    print(f"[DEPTH CAMERA] Current focal length from USD: {current_focal}mm")
                    if current_focal != 1.3:
                        focal_length_attr.Set(1.3)
                        print(f"[DEPTH CAMERA] Focal length corrected to 1.3mm via USD API")
            ...
            print("[DEPTH CAMERA] Position: World coordinates (0.9, 0.16, 1.3)")
            print("[DEPTH CAMERA] Orientation: Euler angles [0.0, 37.0, 90.0] degrees")
            print("[DEPTH CAMERA] Focal Length: 1.3mm (displays as 13.0 in UI)")
    ...
    def _add_initial_obstacles(self):
        ...
        try:
            ...
            print(f"[INIT] Added Obstacle_1 at position {obstacle_1_position}")
            ...
            print(f"[INIT] Added Obstacle_2 at position {obstacle_2_position}")
            print(f"[INIT] Total obstacles loaded: {len(self.obstacles)}")
    ...
    def _add_initial_obstacles_to_rrt(self):
        ...
        try:
            ...
            for obstacle_name, obstacle in self.obstacles.items():
                ...
                print(f"[RRT] Added {obstacle_name} to RRT planner (static={is_static})")

            print(f"[RRT] Total obstacles in RRT planner: {len(self.obstacles)}")
    ...
    def _remove_all_cube_obstacles(self):
        ...
        try:
            for cube, cube_name in self.cubes:
                try:
                    # Check if cube prim still exists before trying to remove it
                    # Picked cubes are deleted from scene, so their prims are invalid
                    if cube.prim and cube.prim.IsValid():
                        self.rrt.remove_obstacle(cube)
                except Exception:
                    # Cube might not be in obstacle list or prim is invalid
                    pass
    ...
    def _process_lidar_data(self):
        ...
        try:
            ...
            # Filter by height (world coordinates) - adjusted for robot at Z=0.6m
            # Cylinders at Z=0.64m, extend to ~0.71725m (0.64 + 0.07725)
            # Filter range: 0.59m to 0.80m (cylinder level obstacles)
            ...
            # Filter out cube pickup region (SAME as franka_rrt_2cubes_v1.5)
            cube_grid_center = np.array([0.45, -0.10])
            cube_grid_margin = 0.30
            ...
            if self.container_dimensions is not None:
                container_pos = np.array([0.4, 0.45, 0.0])  # X: 0.4, Y: 0.45
    ...
    def _move_obstacle(self):
        ...
        Logic:
        - Starts at x=0.85m (right boundary)
        - When x <= 0.65m (left boundary): acceleration = +3 m/s² (move right)
        - When x >= 0.85m (right boundary): acceleration = -3 m/s² (move left)

        IMPORTANT: Only moves Obstacle_1 (the first obstacle created), not Obstacle_2, Obstacle_3, etc.
        """
        # Check if Obstacle_1 exists (prim path is /World/Obstacle_1)
        # The obstacle name in self.obstacles dictionary is "obstacle_1" (lowercase)
        ...
        try:
            ...
            # When at left boundary (x <= 0.65): apply +3 m/s² to move right
            # When at right boundary (x >= 0.85): apply -3 m/s² to move left
            if current_pos[0] <= self.obstacle_1_min_x:
                # At or past left boundary - accelerate right
                acceleration = self.obstacle_1_acceleration  # +3 m/s²
        ...
        try:
            ...
            for i in range(self.current_cube_index, total_cubes):
                try:
                    ...
                    # Extract cylinder number from name (e.g., "Cylinder_1" -> "1")
                    cylinder_num = cube_name.split('_')[1] if '_' in cube_name else str(i + 1)
                    success, error_msg = await self._pick_and_place_cube(cube, cylinder_num)
    ...
    async def _pick_and_place_cube(self, cube, cube_name):
        ...
        try:
            total_cubes = self.grid_length * self.grid_width
            # Cylinders are scaled 1.5x uniformly, so actual height is ~7.725cm
            cylinder_base_height = 0.0515
            cylinder_scale_z = 1.5  # Updated to 1.5
            cylinder_height = cylinder_base_height * cylinder_scale_z  # After scaling = 0.07725m
            ...
            for pick_attempt in range(1, max_pick_attempts + 1):
                ...
                # Add small offset to ensure gripper centers exactly over cylinder
                # This compensates for any end-effector frame offset
                high_waypoint = np.array([
                    cube_pos_current[0] + 0.004,  # 4mm offset in X
                    cube_pos_current[1] + 0.004,  # 4mm offset in Y
                    safe_height
                ])
                ...
                if not success:
                    ...
                    await self._reset_to_safe_config()
                    return False, f"Failed to reach above {cube_name}"

                # Phase 3: Pick approach (descend to exact cylinder center for grasping)
                cube_pos_realtime, _ = cube.get_world_pose()
                # Grasp exactly at cylinder center with small offset to ensure alignment
                pick_pos = np.array([
                ...
                # Slow descent to cylinder center
                ...
                if self.grasp_config and self.grasp_config.is_loaded():
                    ...
                    if grasp_positions:
                        ...
                        # Fallback to default
                        joint_positions = self.gripper.joint_closed_positions
                        print("[GRASP] Using default gripper positions (grasp data not found)")
                else:
                    # Fallback to default
                    joint_positions = self.gripper.joint_closed_positions
                    print("[GRASP] Using default gripper positions (config not loaded)")
                ...
                # Phase 4: Pick retreat - Go straight up from current cylinder position (not EE position)
            ...
            container_center = np.array([0.4, 0.45, 0.0])  # X: 0.4, Y: 0.45
            ...
            if total_cubes <= 4:
                edge_margin_left = 0.10
                edge_margin_right = 0.10
                edge_margin_width = 0.08
            elif total_cubes <= 9:
                edge_margin_left = 0.10
                edge_margin_right = 0.10
                edge_margin_width = 0.08
            else:
                edge_margin_left = 0.08
                edge_margin_right = 0.08
                edge_margin_width = 0.06
            ...
            # Match franka_rrt_2cubes_v1.5.py placement logic:
            # Place height = cylinder_half + 0.005 (relative to world Z=0, not container bottom)
            # This ensures cylinders sit properly on container floor without dipping
            place_height = cylinder_half + 0.005
            ...
            # Use higher waypoints to provide clearance for held cylinder
            ...
            await self._move_to_target_rrt(via_point, orientation, skip_factor=6)
            ...
            await self._move_to_target_rrt(above_place, orientation, skip_factor=6)
            await self._move_to_target_rrt(pre_place_pos, orientation, skip_factor=5)

            # Phase 6: Place approach (faster for efficiency)
            # Release height: 16cm above place position
            release_height = place_pos + np.array([0.0, 0.0, 0.16])
            await self._move_to_target_rrt(release_height, orientation, skip_factor=5)
    ...
    def _plan_to_target(self, target_position, target_orientation):
        ...
        # Update RRT world state to get latest obstacle positions
        # This is CRITICAL for avoiding manually moved obstacles (Obstacle_2, Obstacle_3, etc.)
        self.rrt.update_world()

        # Debug: Print obstacle positions for verification
        if len(self.obstacles) > 0 and not hasattr(self, '_obstacle_debug_logged'):
            print(f"[RRT DEBUG] Planning with {len(self.obstacles)} obstacles:")
            for obs_name, obs_obj in self.obstacles.items():
                obs_pos, _ = obs_obj.get_world_pose()
                print(f"  {obs_name}: position = {obs_pos}")
            self._obstacle_debug_logged = True
    ...
    def _on_add_obstacle(self):
        ...
        try:
            ...
            # Table is at (0.5, 0.1, 0.0) with scale (0.9, 0.7, 0.9)
            # Table dimensions: ~0.9m x 0.7m, so X range: [0.05, 0.95], Y range: [-0.25, 0.45]
            # Place obstacle on table surface at Z=0.75
            # Position within table bounds, avoiding cylinder grid and container
            obstacle_position = np.array([0.55, 0.2, 0.75])  # X: 0.55, Y: 0.2, Z: 0.75
            obstacle_size = np.array([0.20, 0.05, 0.22])  # [length, width, height]
