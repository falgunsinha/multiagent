...
Features:
- RRT path planning for collision-free motion
...
class FrankaRRTDynamicGrid:
    ...
    async def _load_scene(self):
        ...
        try:
            ...
            container_position = np.array([0.45, 0.40, 0.0])
            ...
            cube_size = 0.0515
            cube_spacing = 0.10  # 10cm spacing between cubes for better picking reliability
        ...
        try:
            ...
            for i, (cube, cube_name) in enumerate(cubes, 1):
                ...
                if success:
                    ...
                    print(f"\n=== FAILED: Only {self.placed_count}/{total_cubes} cubes placed ===\n")
                    self._update_status(f"{error_msg} - {self.placed_count}/{total_cubes} placed")
                    self.is_picking = False
                    return

            print(f"\n=== SUCCESS: ALL {self.placed_count}/{total_cubes} CUBES PLACED! ===\n")
            self._update_status(f"All {self.placed_count} cubes placed successfully!")
            self.is_picking = False
            self.timeline.stop()
    ...
    async def _pick_and_place_cube(self, cube, cube_name, is_last_cube=False):
        """
        Pick and place a single cube using clean state machine (UR10 pattern)

        PHASE 1: Travel to pre-pick (RRT)
        PHASE 2: Pick approach - open gripper, descend straight down (IK)
        PHASE 3: Pick retreat - close gripper, ascend straight up (IK)
        PHASE 4: Travel to pre-place (RRT)
        PHASE 5: Place approach - descend straight down (IK)
        PHASE 6: Place retreat - open gripper, ascend straight up (IK)
        PHASE 7: Return to home (only if last cube)
        ...
        try:
            ...
            print(f"\n[PICK] Cube {cube_name} at position: {cube_pos}")

            # Calculate key positions
            cube_size = 0.0515
            cube_half = cube_size / 2.0

            # Pre-pick position: 12cm above cube (reduced from 15cm for better reach)
            pre_pick_pos = cube_pos + np.array([0.0, 0.0, 0.12])

            # Pick position: 2cm from ground (below cube center for better grip)
            ...
            print(f"  → Spacing: Length={spacing_length:.3f}m, Width={spacing_width:.3f}m")
            print(f"  → Edge margin: {edge_margin:.3f}m, Start: X={start_x:.3f}m, Y={start_y:.3f}m")
            print(f"  → Container edges: X=[{container_left_edge:.3f}, {container_right_edge:.3f}], Y=[{container_front_edge:.3f}, {container_back_edge:.3f}]")
            ...
            # Pre-place position: 25cm above place location (increased to avoid container rim)
            ...
            if not success:
                ...
                return False, f"Failed to reach pre-pick position for {cube_name}"
            ...
            # Descend straight down to pick position (IK)
            ...
            if not success:
                ...
                return False, f"Failed to descend to pick position for {cube_name}"
            ...
            # Verify pick
            cube_pos_after, _ = cube.get_world_pose()
            if abs(cube_pos_after[2] - cube_pos[2]) < 0.001:
                print(f"   [WARN] Cube may not be picked! Position unchanged")
            else:
                print(f"   Gripper: OK - Cube lifted to Z={cube_pos_after[2]:.4f}")
            ...
            current_ee_pos, _ = self.franka.end_effector.get_world_pose()
            retreat_pos = current_ee_pos + np.array([0.0, 0.0, 0.18])  # 18cm retreat (higher for safer transit)
            ...
            if not success:
                ...
                return False, f"Failed to retreat after picking {cube_name}"
            ...
            if not success:
                ...
                return False, f"Failed to reach pre-place position for {cube_name}"
            ...
            success = await self._taskspace_straight_line(current_ee_pos, release_height, orientation, num_waypoints=20)
            if not success:
                print("[ERROR] Failed place approach")
                return False, f"Failed to descend to place position for {cube_name}"
            ...
            current_ee_pos, _ = self.franka.end_effector.get_world_pose()
            retreat_pos = current_ee_pos + np.array([0.0, 0.0, 0.20])  # 20cm retreat (increased to clear container rim)
            success = await self._taskspace_straight_line(current_ee_pos, retreat_pos, orientation, num_waypoints=20)
            if not success:
                print("[ERROR] Failed place retreat")
                return False, f"Failed to retreat after placing {cube_name}"
            ...
            error_msg = f"Error picking/placing {cube_name}: {str(e)}"
            print(error_msg)
            import traceback
            traceback.print_exc()
    ...
    def _plan_to_target(self, target_position, target_orientation):
        ...
        robot_base_translation, robot_base_orientation = self.franka.get_world_pose()
        self.kinematics_solver.set_robot_base_pose(robot_base_translation, robot_base_orientation)
        self.rrt.set_robot_base_pose(robot_base_translation, robot_base_orientation)  # CRITICAL: Update RRT's robot base pose
        ...
        if not ik_success:
            ...
            print(f"  ✓ IK validation successful for target {target_position}")
    ...
    async def _execute_plan(self, plan):
        ...
        # Add slight delay every few actions for smoother motion without major FPS drop
        for i, action in enumerate(plan):
            self.franka.apply_action(action)
            await omni.kit.app.get_app().next_update_async()

            # Add extra wait every 3rd action for slight slowdown
            if i % 3 == 0:
                await omni.kit.app.get_app().next_update_async()
