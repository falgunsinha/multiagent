...
class FrankaRRTDynamicGrid:
    ...
    async def _pick_and_place_cube(self, cube, cube_name, is_last_cube=False):
        """
        Pick and place a single cube using clean state machine with PICK retry logic
        ...
        - PHASE 6: Place approach - descend straight down (IK)
        - PHASE 7: Place retreat - open gripper, ascend straight up (IK)
        ...
        try:
            ...
            # Release height: 5cm above final position
            release_height = place_pos + np.array([0.0, 0.0, 0.05])
    ...
    async def _execute_plan(self, plan):
        ...
        Args:
            plan: RRT plan (list of ArticulationActions)
        """
        if plan is None:
            return False
