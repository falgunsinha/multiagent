"""
Franka RRT Pick and Place - Dynamic Grid with Kinematics Solver + Bounding Box Detection
...
SENSORS:
- PhysX Lidar - Rotating: Dynamic obstacle detection (60Hz)
- Intel RealSense D455 depth sensor: Dynamic obstacle detection (optional)
- Main_Camera with Replicator: Bounding box detection for object categorization

BOUNDING BOX DETECTION:
- Uses Replicator's bounding_box_3d annotator on Main_Camera
- Categorizes objects by size:
  * Small cubes (~0.0515m) -> Cubes to pick (semantic label: "small_cube")
  * Large cubes (>0.15m) -> Obstacles to avoid (semantic label: "large_obstacle")
- Outputs JSON to _out_bbox_detection directory
- Replaces static cube selection with dynamic detection

CONTAINER & PLACEMENT:
- Container position and placement grid remain statically defined
- Only cubes and obstacles are detected dynamically
"""
...
# Replicator imports for bounding box detection
...
class FrankaRRTDynamicGrid:
    ...
    def __init__(self):
        ...
        # Replicator for bounding box detection
        self.render_product = None  # Render product for Main_Camera
        self.bbox_annotator = None  # Bounding box 3D annotator
        self.bbox_writer = None  # Writer for bounding box JSON output
        self.detected_cubes = []  # List of detected cubes from bounding boxes (small objects)
        self.detected_obstacles = []  # List of detected obstacles from bounding boxes (large objects)
            ...
            for row in range(self.grid_length):
                for col in range(self.grid_width):
                    ...
                    if prim:
                        ...
                        # Add semantic label for bounding box detection (small cubes to pick)
                        add_labels(prim, labels=["small_cube"], instance_name="class")
                        print(f"  Added semantic label 'small_cube' to {prim_path}")
    ...
    def _setup_bounding_box_detection(self):
        """
        Setup Replicator bounding box detection using Main_Camera.
        This should be called AFTER the simulation is running (timeline.play()).
        """
        try:
            print("[BBOX] Setting up bounding box detection with Main_Camera...")

            # Disable capture on play to have manual control
            rep.orchestrator.set_capture_on_play(False)

            # Create render product from Main_Camera
            camera_prim_path = "/World/Main_Camera"
            self.render_product = rep.create.render_product(camera_prim_path, (512, 512), name="main_camera_view")
            print(f"[BBOX] Created render product for {camera_prim_path}")

            # Attach bounding box 3D annotator
            self.bbox_annotator = rep.annotators.get("bounding_box_3d")
            self.bbox_annotator.attach(self.render_product)
            print("[BBOX] Attached bounding_box_3d annotator")

            # Setup BasicWriter for JSON output
            out_dir = os.path.join(os.getcwd(), "_out_bbox_detection")
            print(f"[BBOX] Output directory: {out_dir}")

            self.bbox_writer = rep.WriterRegistry.get("BasicWriter")
            self.bbox_writer.initialize(output_dir=out_dir, rgb=False, bounding_box_3d=True)
            self.bbox_writer.attach([self.render_product])
            print("[BBOX] BasicWriter initialized and attached")

            print("[BBOX] Bounding box detection setup complete!")

        except Exception as e:
            carb.log_warn(f"[BBOX ERROR] Error setting up bounding box detection: {e}")
            import traceback
            traceback.print_exc()

    async def _process_bounding_boxes(self):
        """
        Process bounding box data to categorize objects:
        - Small cubes (0.0515m scale) -> cubes to pick
        - Large cubes (>0.15m scale) -> obstacles to avoid

        Returns:
            (detected_cubes, detected_obstacles) - Lists of dictionaries with 'position', 'size', 'prim_path'
        """
        try:
            if self.bbox_annotator is None:
                print("[BBOX] Bounding box annotator not initialized")
                return [], []

            # Trigger a data capture
            ...
            if bbox_data is None or len(bbox_data) == 0:
                print("[BBOX] No bounding box data available")
                return [], []

            print(f"\n[BBOX] Processing bounding box data...")
            print(f"[BBOX] Raw data keys: {bbox_data.keys() if hasattr(bbox_data, 'keys') else 'N/A'}")

            detected_cubes = []
            detected_obstacles = []

            # Parse bounding box data
            # The data structure is: {'data': array, 'info': {...}}
            if 'data' in bbox_data:
                bbox_array = bbox_data['data']
                info = bbox_data.get('info', {})

                print(f"[BBOX] Bounding boxes found: {len(bbox_array) if bbox_array is not None else 0}")

                # Each bounding box has: semanticId, x_min, y_min, z_min, x_max, y_max, z_max, transform, occlusion
                for i, bbox in enumerate(bbox_array):
                    try:
                        ...
                        # Average size for classification
                        avg_size = (size_x + size_y + size_z) / 3.0

                        # Categorize based on size
                        # Small cubes: ~0.0515m (pickable cubes)
                        # Large cubes: >0.15m (obstacles)
                        if avg_size < 0.10:  # Small cube threshold
                            detected_cubes.append({
                                'position': np.array([center_x, center_y, center_z]),
                                'size': np.array([size_x, size_y, size_z]),
                                'avg_size': avg_size
                            })
                            print(f"[BBOX] Small cube #{len(detected_cubes)}: pos=({center_x:.3f}, {center_y:.3f}, {center_z:.3f}), size={avg_size:.3f}m")
                        else:  # Large obstacle
                            ...
                            print(f"[BBOX] Large obstacle #{len(detected_obstacles)}: pos=({center_x:.3f}, {center_y:.3f}, {center_z:.3f}), size={avg_size:.3f}m")

                    except Exception as bbox_error:
                        print(f"[BBOX] Error processing bbox {i}: {bbox_error}")
                        continue

            print(f"[BBOX] Detection complete: {len(detected_cubes)} cubes, {len(detected_obstacles)} obstacles")
            return detected_cubes, detected_obstacles

        except Exception as e:
            carb.log_warn(f"[BBOX ERROR] Error processing bounding boxes: {e}")
            import traceback
            traceback.print_exc()
            return [], []
    ...
    async def _pick_place_loop(self):
        ...
        try:
            ...
            # Setup bounding box detection (if not already setup)
            if self.bbox_annotator is None:
                print("[BBOX] Setting up bounding box detection (first run)...")
                self._setup_bounding_box_detection()
                # Wait for setup to complete
                for _ in range(10):
                    await omni.kit.app.get_app().next_update_async()

            # DYNAMIC CUBE DETECTION: Use bounding boxes to detect cubes instead of static list
            print("\n[BBOX] Detecting cubes and obstacles using bounding boxes...")
            detected_cubes_bbox, detected_obstacles_bbox = await self._process_bounding_boxes()

            # Store detected obstacles for RRT (will be added as static obstacles)
            self.detected_obstacles = detected_obstacles_bbox if detected_obstacles_bbox else []

            # Add detected large obstacles to RRT planner
            if self.detected_obstacles and len(self.detected_obstacles) > 0:
                print(f"[BBOX] Adding {len(self.detected_obstacles)} large obstacles to RRT planner...")
                for i, obs_bbox in enumerate(self.detected_obstacles):
                    obs_name = f"bbox_obstacle_{i}"
                    obs_prim_path = f"/World/BBoxObstacle_{i}"
                    obs_pos = obs_bbox['position']
                    obs_size = obs_bbox['size']
                    ...
                    if obs_name not in self.obstacles:
                        ...
                        obstacle = self.world.scene.add(
                            FixedCuboid(
                                ...
                                size=1.0,
                                scale=obs_size,
                                color=np.array([1.0, 0.5, 0.0]),  # Orange for bbox obstacles
                                visible=True
                            )
                        )
                        self.rrt.add_obstacle(obstacle, static=True)
                        self.obstacles[obs_name] = obstacle
                        print(f"[BBOX] Added obstacle: {obs_name} at ({obs_pos[0]:.3f}, {obs_pos[1]:.3f}, {obs_pos[2]:.3f})")

            # Match detected cubes with scene objects
            cubes_to_pick = []
            if detected_cubes_bbox and len(detected_cubes_bbox) > 0:
                print(f"[BBOX] Found {len(detected_cubes_bbox)} cubes to pick")

                # Match detected positions with actual cube objects
                for bbox_cube in detected_cubes_bbox:
                    bbox_pos = bbox_cube['position']

                    # Find closest cube object to this bounding box position
                    min_dist = float('inf')
                    closest_cube = None

                    for cube, cube_name in self.cubes:
                        cube_pos, _ = cube.get_world_pose()
                        dist = np.linalg.norm(cube_pos - bbox_pos)
                        if dist < min_dist:
                            min_dist = dist
                            closest_cube = (cube, cube_name)

                    # If we found a close match (within 10cm), add it
                    if closest_cube and min_dist < 0.10:
                        cubes_to_pick.append(closest_cube)
                        print(f"[BBOX] Matched cube: {closest_cube[1]} at distance {min_dist:.3f}m")
            else:
                print("[BBOX] No cubes detected via bounding boxes, using static list as fallback")
                cubes_to_pick = self.cubes

            total_cubes = len(cubes_to_pick)
            print(f"[BBOX] Total cubes to pick: {total_cubes}")

            for i in range(self.current_cube_index, total_cubes):
                try:
                    cube, cube_name = cubes_to_pick[i]
                    cube_number = i + 1

                    # Get current position from bounding box detection
                    cube_pos_current, _ = cube.get_world_pose()
                    print(f"[{cube_number}/{total_cubes}] {cube_name} at ({cube_pos_current[0]:.3f}, {cube_pos_current[1]:.3f}, {cube_pos_current[2]:.3f})")
    ...
    def _on_add_obstacle(self):
        ...
        try:
            ...
            # Add semantic label for bounding box detection (large obstacles to avoid)
            stage = omni.usd.get_context().get_stage()
            obstacle_prim = stage.GetPrimAtPath(obstacle_prim_path)
            if obstacle_prim:
                add_labels(obstacle_prim, labels=["large_obstacle"], instance_name="class")
                print(f"  Added semantic label 'large_obstacle' to {obstacle_prim_path}")
