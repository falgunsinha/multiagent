...
class FrankaRRTDynamicGrid:
    ...
    def __init__(self):
        ...
        # Visual update tracking for continuous updates
        self.visual_update_counter = 0
        self.visual_update_interval = 3  # Update visuals every 3 frames for real-time feedback
    ...
    def _process_lidar_data(self):
        ...
        try:
            ...
            total_raw_points = len(points_world)
            ...
            after_height_filter = len(valid_points)
            ...
            after_distance_filter = len(valid_points)

            # Filter out cube pickup region (REDUCED margin to detect obstacles closer to cubes)
            cube_grid_center = np.array([0.45, -0.10])
            cube_grid_margin = 0.20  # Reduced from 0.30 to 0.20 for better obstacle detection near cubes
            ...
            after_cube_filter = len(valid_points)
            ...
            after_container_filter = len(valid_points)
            ...
            after_robot_filter = len(valid_points)

            # Perform clustering FIRST before logging
            detected_obstacles = []
            ...
            if len(valid_points) > 10:
                ...
                # Reduced threshold from 5 to 3 points for faster detection
                # This allows obstacles to be detected more quickly when moved
                obstacle_cells = unique_cells[counts > 3]
                detected_obstacles = obstacle_cells.tolist()

            # Debug logging for filter stages (ALWAYS log when significant filtering happens)
            if total_raw_points > 100:
                if after_robot_filter < 50:
                    print(f"[LIDAR FILTER] Raw:{total_raw_points} → Height:{after_height_filter} → Dist:{after_distance_filter} → Cube:{after_cube_filter} → Container:{after_container_filter} → Robot:{after_robot_filter}")
                elif after_robot_filter > 0 and len(detected_obstacles) == 0:
                    # Points survived all filters but no obstacles detected (clustering issue)
                    print(f"[LIDAR FILTER] Raw:{total_raw_points} → Final:{after_robot_filter} points, but 0 obstacles (clustering threshold not met)")
                ...
                # Log detected obstacles with actual names from stage (ALWAYS log for debugging)
                # Also check actual obstacle positions from stage for comparison
                from omni.isaac.core.utils.stage import get_current_stage
                from omni.isaac.core.prims import XFormPrim
                stage = get_current_stage()

                actual_obstacle_positions = {}
                for prim in stage.Traverse():
                    prim_path = str(prim.GetPath())
                    if "/World/Obstacle_" in prim_path and "/World/LidarObstacle_" not in prim_path:
                        try:
                            xform = XFormPrim(prim_path)
                            prim_pos, _ = xform.get_world_pose()
                            obs_name = prim_path.split('/')[-1]
                            actual_obstacle_positions[obs_name] = prim_pos
                        except:
                            pass
                ...
                # Log actual obstacle positions that were NOT detected
                if len(actual_obstacle_positions) > 0:
                    detected_positions_set = set()
                    for obs_pos in detected_obstacles:
                        for obs_name, actual_pos in actual_obstacle_positions.items():
                            dist_xy = np.linalg.norm(np.array(obs_pos[:2]) - actual_pos[:2])
                            if dist_xy < 0.20:
                                detected_positions_set.add(obs_name)

                    for obs_name, actual_pos in actual_obstacle_positions.items():
                        if obs_name not in detected_positions_set:
                            print(f"  [NOT DETECTED] {obs_name} at ({actual_pos[0]:.2f}, {actual_pos[1]:.2f}, {actual_pos[2]:.2f})m")

                # COOL VISUAL EFFECTS: RED RAYS from Lidar to Obstacle
                if self.debug_draw:
                    # ALWAYS clear previous drawings for smooth animation (even when no obstacles)
                    ...
                    if len(detected_obstacles) > 0:
                        ...
                        for obs_pos in detected_obstacles[:10]:  # Limit to 10 for performance
                            # Draw RED TRIANGULAR BEAM from Lidar to obstacle SURFACE
                            # Cone/triangle shape that spreads from Lidar (narrow) to obstacle surface (wide)

                            # Calculate direction vector to obstacle center
                            direction = np.array(obs_pos) - lidar_pos
                            distance_to_obstacle_center = np.linalg.norm(direction)
                            direction_norm = direction / (distance_to_obstacle_center + 1e-6)

                            # Calculate distance to obstacle SURFACE (not center)
                            # Obstacles are 15cm cubes, so radius is ~10.6cm (half diagonal in XY plane)
                            # Use conservative 8cm to account for cube geometry
                            obstacle_radius = 0.08  # 8cm from center to surface
                            distance_to_surface = max(0.1, distance_to_obstacle_center - obstacle_radius)

                            # Ray end point at obstacle SURFACE (not center)
                            surface_pos = lidar_pos + direction_norm * distance_to_surface
                            ...
                            # Triangular beam: narrow at Lidar, wide at obstacle surface
                            ...
                            # Beam width at obstacle surface (wide spread)
                            # Scale width based on distance for consistent visual appearance
                            obstacle_width = 0.12  # 12cm at obstacle surface (wide end)
                            ...
                            for i in range(num_rays):
                                ...
                                # Triangular spread: narrow at Lidar, wide at obstacle surface
                                ...
                                # End point at obstacle SURFACE (not center - rays stop at surface)
                                end_pt = surface_pos + end_offset
                            ...
                            # Left edge: from Lidar left to obstacle surface left
                            left_start = lidar_pos + (-lidar_width/2) * perp
                            left_end = surface_pos + (-obstacle_width/2) * perp

                            # Right edge: from Lidar right to obstacle surface right
                            right_start = lidar_pos + (lidar_width/2) * perp
                            right_end = surface_pos + (obstacle_width/2) * perp

                            # Base edge: obstacle surface left to obstacle surface right
                            base_start = surface_pos + (-obstacle_width/2) * perp
                            base_end = surface_pos + (obstacle_width/2) * perp
    ...
    def _update_lidar_visuals_only(self):
        ...
        try:
            # Call _process_lidar_data() which updates visuals
            # But don't update RRT obstacles (that's done in _update_dynamic_obstacles)
            self._process_lidar_data()
        except Exception as e:
            pass  # Silently ignore visual update errors
    ...
    def _periodic_visual_update(self):
        ...
        """
        self.visual_update_counter += 1
        if self.visual_update_counter >= self.visual_update_interval:
            self.visual_update_counter = 0
            self._update_lidar_visuals_only()

    def _update_dynamic_obstacles(self):
        """
        Update RRT planner with dynamically detected obstacles from Lidar.

        OPTIMIZED with POSITION-BASED TRACKING:
        1. Match detected obstacles to existing prims by proximity (not index)
        2. Move matched obstacles to new positions
        3. Create new prims for unmatched detections
        4. Remove prims that are no longer detected
        This ensures obstacles are tracked correctly even when they move
        """
        ...
        try:
            ...
            if len(detected_positions) == 0:
                # No obstacles detected - remove all existing ones
            ...
            # POSITION-BASED MATCHING: Match detected positions to existing obstacles
            # This ensures obstacles are tracked correctly even when they move
            matched_obstacles = {}  # {obs_name: new_position}
            unmatched_detections = []  # List of detected positions without matches

            # For each detected position, find the closest existing obstacle
            for detected_pos in detected_positions:
                detected_pos = np.array(detected_pos)
                best_match = None
                best_distance = float('inf')

                # Find closest existing obstacle (within 40cm - reasonable movement range)
                for obs_name, obs_obj in self.lidar_detected_obstacles.items():
                    if obs_name in matched_obstacles:
                        continue  # Already matched
                    current_pos, _ = obs_obj.get_world_pose()
                    distance = np.linalg.norm(detected_pos - current_pos)

                    # Match if within 40cm (allows for obstacle movement)
                    if distance < 0.40 and distance < best_distance:
                        best_match = obs_name
                        best_distance = distance

                if best_match is not None:
                    # Found a match - update this obstacle's position
                    matched_obstacles[best_match] = detected_pos
                else:
                    # No match found - this is a new obstacle
                    unmatched_detections.append(detected_pos)

            # Update matched obstacles to their new positions
            for obs_name, new_pos in matched_obstacles.items():
                obs_obj = self.lidar_detected_obstacles[obs_name]
                current_pos, _ = obs_obj.get_world_pose()

                # ALWAYS update position for real-time tracking (no threshold)
                # This ensures obstacles are tracked immediately when moved
                obs_obj.set_world_pose(position=new_pos)
                self.rrt.update_world()

            # Remove unmatched obstacles (no longer detected)
            for obs_name in list(self.lidar_detected_obstacles.keys()):
                if obs_name not in matched_obstacles:
                    obs_obj = self.lidar_detected_obstacles[obs_name]
                    try:
                        self.rrt.remove_obstacle(obs_obj)
                        self.world.scene.remove_object(obs_name)
            ...
            for i, new_pos in enumerate(unmatched_detections):
                # Find available obstacle index
                existing_indices = [int(name.split('_')[-1]) for name in self.lidar_detected_obstacles.keys()]
                new_index = 0
                while new_index in existing_indices:
                    new_index += 1

                obs_name = f"lidar_obstacle_{new_index}"
                obs_prim_path = f"/World/LidarObstacle_{new_index}"
    ...
    def _plan_to_target(self, target_position, target_orientation):
        ...
        has_obstacles = self.obstacle_counter > 0 or len(self.lidar_detected_obstacles) > 0
        ...
        if rrt_plan is None or len(rrt_plan) <= 1:
            carb.log_warn(f"RRT failed for {target_position} (obstacles: {len(self.lidar_detected_obstacles)})")
            return None
