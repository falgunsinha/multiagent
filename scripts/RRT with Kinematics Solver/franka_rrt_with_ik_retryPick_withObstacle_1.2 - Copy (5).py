...
class FrankaRRTDynamicGrid:
    ...
    async def _pick_and_place_cube(self, cube, cube_name, is_last_cube=False):
        ...
        try:
            ...
            # Calculate placement position inside container - CENTER ALL CUBES
            ...
            # Cube spacing: Small gap between cubes (8cm center-to-center)
            # Cube is 5.15cm, so 8cm spacing means ~2.85cm gap between cubes
            cube_spacing = 0.08  # 8cm between cube centers

            # Calculate total grid size
            total_grid_length = (self.grid_length - 1) * cube_spacing if self.grid_length > 1 else 0.0
            total_grid_width = (self.grid_width - 1) * cube_spacing if self.grid_width > 1 else 0.0

            # Calculate starting position to center the grid in container
            # Start from container center and offset by half the grid size
            start_x = container_center[0] - total_grid_length / 2.0
            start_y = container_center[1] - total_grid_width / 2.0

            # Calculate cube position in grid
            cube_x = start_x + (place_row * cube_spacing)
            cube_y = start_y + (place_col * cube_spacing)
            ...
            print(f"  - Cube spacing: {cube_spacing:.4f}m ({cube_spacing*100:.1f}cm center-to-center)")
            print(f"  - Grid centered at: [{container_center[0]:.4f}, {container_center[1]:.4f}]")
            print(f"  - Grid size: {total_grid_length:.4f}m x {total_grid_width:.4f}m")
            ...
            # Pre-place position: Height depends on distance from robot
            # Far positions (X > 0.5) need lower height to stay within reach
            # Obstacle at [0.27, 0.17, 0.19] requires clearance during vertical descent
            if place_pos[0] > 0.5:  # Far position (back row)
                # Use 25cm height for far positions to stay within robot reach
                pre_place_height = 0.25
                print(f"   [FAR POSITION] Using reduced pre-place height: {pre_place_height}m")
            else:  # Near position (front row)
                # Use 40cm height for near positions to clear obstacle
                pre_place_height = 0.40

            pre_place_pos = place_pos + np.array([0.0, 0.0, pre_place_height])
            ...
            # Release height: Just above cube height to avoid gripper hitting other cubes
            # Cube height is 5.15cm, so release at 9cm above container floor
            # This gives 3.85cm clearance above cubes already in container
            release_height_z = 0.09  # 9cm above container floor
            release_height = np.array([place_pos[0], place_pos[1], release_height_z])

            # Descend to 10cm above final position (safe clearance, avoids gripper hitting rim)
            success = await self._move_to_target_ik(release_height, orientation, num_steps=30)
            if not success:
