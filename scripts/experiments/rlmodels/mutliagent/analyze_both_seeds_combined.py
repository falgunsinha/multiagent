...
# First pass: collect all data and find max distance/time per episode for baseline
df_combined_all = pd.concat([df_42, df_123])
baseline_distance_per_episode = df_combined_all.groupby('episode')['total_distance_reduced'].max()
baseline_time_per_episode = df_combined_all.groupby('episode')['total_time_saved'].max()
...
for model in model_order:
    ...
    # Calculate efficiency for seed 42
    efficiency_distance_42 = []
    efficiency_time_42 = []
    for _, row in model_df_42.iterrows():
        ep = row['episode']
        baseline_dist = baseline_distance_per_episode[ep]
        baseline_t = baseline_time_per_episode[ep]
        efficiency_distance_42.append((row['total_distance_reduced'] / baseline_dist) * 100 if baseline_dist > 0 else 0)
        efficiency_time_42.append((row['total_time_saved'] / baseline_t) * 100 if baseline_t > 0 else 0)

    distance_efficiency_42 = np.mean(efficiency_distance_42)
    time_efficiency_42 = np.mean(efficiency_time_42)
    ...
    # Calculate efficiency for seed 123
    efficiency_distance_123 = []
    efficiency_time_123 = []
    for _, row in model_df_123.iterrows():
        ep = row['episode']
        baseline_dist = baseline_distance_per_episode[ep]
        baseline_t = baseline_time_per_episode[ep]
        efficiency_distance_123.append((row['total_distance_reduced'] / baseline_dist) * 100 if baseline_dist > 0 else 0)
        efficiency_time_123.append((row['total_time_saved'] / baseline_t) * 100 if baseline_t > 0 else 0)

    distance_efficiency_123 = np.mean(efficiency_distance_123)
    time_efficiency_123 = np.mean(efficiency_time_123)
    ...
    distance_efficiency_mean = np.mean([distance_efficiency_42, distance_efficiency_123])
    distance_efficiency_std = np.std([distance_efficiency_42, distance_efficiency_123], ddof=1)

    time_efficiency_mean = np.mean([time_efficiency_42, time_efficiency_123])
    time_efficiency_std = np.std([time_efficiency_42, time_efficiency_123], ddof=1)
    ...
    results.append({
        ...
        'distance_efficiency_mean': distance_efficiency_mean,
        'distance_efficiency_std': distance_efficiency_std,
        'time_efficiency_mean': time_efficiency_mean,
        'time_efficiency_std': time_efficiency_std
    })

# Print table
print(f"\n{'Model':<25} {'Reward':>15} {'Pick Success (%)':>25} {'Distance Efficiency (%)':>30} {'Time Efficiency (%)':>30}")
print("-"*140)
...
for result in results:
    ...
    distance_eff_str = f"{result['distance_efficiency_mean']:.2f} ± {result['distance_efficiency_std']:.2f}"
    time_eff_str = f"{result['time_efficiency_mean']:.2f} ± {result['time_efficiency_std']:.2f}"

    print(f"{model_name:<25} {reward_str:>15} {success_str:>25} {distance_eff_str:>30} {time_eff_str:>30}")
...
max_distance_eff = max([r['distance_efficiency_mean'] for r in results])
max_time_eff = max([r['time_efficiency_mean'] for r in results])
...
for result in results:
    ...
    distance_eff_val = f"{result['distance_efficiency_mean']:.2f} $\\pm$ {result['distance_efficiency_std']:.2f}"
    if result['distance_efficiency_mean'] == max_distance_eff or is_heuristic:
        distance_eff_str = f"\\textbf{{{distance_eff_val}}}"
    else:
        distance_eff_str = distance_eff_val

    # Time Efficiency column
    time_eff_val = f"{result['time_efficiency_mean']:.2f} $\\pm$ {result['time_efficiency_std']:.2f}"
    if result['time_efficiency_mean'] == max_time_eff or is_heuristic:
...
# Set up font settings (1.5x from original)
plt.rcParams.update({
    'font.size': 21,           # Base font size (14 * 1.5)
    'axes.labelsize': 24,      # X and Y axis titles (16 * 1.5)
    'axes.titlesize': 27,      # Graph title (18 * 1.5)
    'xtick.labelsize': 21,     # X axis values (14 * 1.5)
    'ytick.labelsize': 21,     # Y axis values (14 * 1.5)
    'legend.fontsize': 21,     # Legend names (14 * 1.5)
})

# Define custom color palette: light green from epsilon decay for DDQN + mGAT, light blue for Duel-DDQN + SAC
custom_colors = ['#B8CD50', '#607D8B', '#40E0D0']  # Light green (epsilon decay), Light blue/slate, Turquoise
...
# Set labels and title with bold (using rcParams font sizes)
ax.set_xlabel('Episode', fontweight='bold')
ax.set_ylabel('Reward', fontweight='bold')
ax.set_title('Reward by episode', fontweight='bold')
...
ax.set_xticklabels(episodes_to_plot)
...
# Define custom color palette: same light green for DDQN + mGAT as reward graph (epsilon decay), gray for Heuristic
custom_colors_success = {
    'DDQN + mGAT': '#B8CD50',  # light green (same as reward graph - from epsilon decay)
    'Heuristic': '#808080'  # gray
}

# Define markers
markers_success = {
    'DDQN + mGAT': 'o',  # circle
    'Heuristic': 'x'  # cross (will be bold with markeredgewidth)
}
...
for model in ['DDQN + mGAT', 'Heuristic']:
    ...
    # Dashed line for Heuristic, solid for DDQN + mGAT
    if model == 'Heuristic':
        ax.plot(x, y, color=custom_colors_success[model], linewidth=4, linestyle='--',
                label=model, marker=markers_success[model], markersize=10, markevery=1, markeredgewidth=2)
    else:
...
ax.set_title('Success rate by models', fontweight='bold')
...
ax.set_xticklabels(x_ticks_display)
...
for model in ['DDQN + mGAT', 'Heuristic']:
    ...
    # Dashed line for Heuristic, solid for DDQN + mGAT
    if model == 'Heuristic':
        ax.plot(x, y, color=custom_colors_distance[model], linewidth=4, linestyle='--',
                label=model, marker=markers_distance[model], markersize=10, markevery=1, markeredgewidth=2)
    else:
...
# Set labels and title with bold (using rcParams font sizes)
ax.set_xlabel('Episode', fontweight='bold')
ax.set_ylabel('Distance reduced (m)', fontweight='bold')
ax.set_title('Distance reduced by models', fontweight='bold')

# Customize legend - moved to upper left
...
for model in ['DDQN + mGAT', 'Heuristic']:
    ...
    # Dashed line for Heuristic, solid for DDQN + mGAT
    if model == 'Heuristic':
        ax.plot(x, y, color=custom_colors_time[model], linewidth=4, linestyle='--',
                label=model, marker=markers_time[model], markersize=10, markevery=1, markeredgewidth=2)
    else:
...
# Set labels and title with bold (using rcParams font sizes)
ax.set_xlabel('Episode', fontweight='bold')
ax.set_ylabel('Time saved (s)', fontweight='bold')
ax.set_title('Time saved by models', fontweight='bold')

# Customize legend - moved to upper left
ax.legend(frameon=True, loc='upper left')

# Set y-axis limits and ticks
ax.set_ylim(bottom=0.0, top=3.2)
...
# ============================================================================
# 5. DISTANCE EFFICIENCY BY EPISODE (DDQN + mGAT vs Heuristic)
# ============================================================================
print("\n" + "="*80)
print("Generating Distance Efficiency graph...")
print("="*80)

# Calculate distance efficiency using maximum distance_reduced as baseline
df_efficiency_distance = df_combined[df_combined['model'].isin(['DDQN+MASAC', 'Heuristic'])].copy()
df_efficiency_distance['model_display'] = df_efficiency_distance['model'].map({
    'DDQN+MASAC': 'DDQN + mGAT',
    'Heuristic': 'Heuristic'
})

# Filter for all episodes from 1 to 17
df_filtered_efficiency_distance = df_efficiency_distance[df_efficiency_distance['episode'].isin(all_episodes)].copy()

# For each episode, calculate the maximum distance_reduced across all models (baseline)
df_all_models = df_combined[df_combined['episode'].isin(all_episodes)].copy()
baseline_distance = df_all_models.groupby('episode')['total_distance_reduced'].max().reset_index()
baseline_distance.columns = ['episode', 'baseline_distance']

# Calculate average distance for each model at each episode
df_grouped_efficiency_distance = df_filtered_efficiency_distance.groupby(['model_display', 'episode'])['total_distance_reduced'].mean().reset_index()

# Merge with baseline
df_grouped_efficiency_distance = df_grouped_efficiency_distance.merge(baseline_distance, on='episode')

# Calculate efficiency as percentage: (distance_reduced / baseline) * 100
df_grouped_efficiency_distance['efficiency'] = (df_grouped_efficiency_distance['total_distance_reduced'] / df_grouped_efficiency_distance['baseline_distance']) * 100

# Pivot to get DDQN+mGAT and Heuristic as separate series
df_pivot_distance = df_grouped_efficiency_distance.pivot(index='episode', columns='model_display', values='efficiency')
...
# Plot efficiency lines for both models
x = df_pivot_distance.index.values
y_ddqn = df_pivot_distance['DDQN + mGAT'].values
y_heuristic = df_pivot_distance['Heuristic'].values

ax.plot(x, y_ddqn, color='#B8CD50', linewidth=4, label='DDQN + mGAT', marker='o', markersize=10, markevery=1)
ax.plot(x, y_heuristic, color='#808080', linewidth=4, label='Heuristic', marker='x', markersize=10, markevery=1, linestyle='--', markeredgewidth=2)

# Customize grid - solid lines, thicker, more visible
ax.grid(True, linestyle='-', linewidth=1.5, alpha=0.7)
ax.set_axisbelow(True)

# Set labels and title with bold
ax.set_xlabel('Episode', fontweight='bold')
ax.set_ylabel('Distance Efficiency (%)', fontweight='bold')
ax.set_title('Distance efficiency by models', fontweight='bold')
...
# Save figure
output_path_distance_eff = Path("cobotproject/scripts/experiments/rlmodels/mutliagent/two_agent_results/discrete/distance_efficiency_by_episode.png")
output_path_distance_eff.parent.mkdir(parents=True, exist_ok=True)
plt.savefig(output_path_distance_eff, dpi=300, bbox_inches='tight')
print(f"✓ Line chart saved to: {output_path_distance_eff}")
plt.close()


# ============================================================================
# 6. TIME EFFICIENCY BY EPISODE (DDQN + mGAT vs Heuristic)
# ============================================================================
print("\n" + "="*80)
print("Generating Time Efficiency graph...")
print("="*80)

# Calculate time efficiency using maximum time_saved as baseline
df_efficiency_time = df_combined[df_combined['model'].isin(['DDQN+MASAC', 'Heuristic'])].copy()
df_efficiency_time['model_display'] = df_efficiency_time['model'].map({
    'DDQN+MASAC': 'DDQN + mGAT',
    'Heuristic': 'Heuristic'
})

# Filter for all episodes from 1 to 17
df_filtered_efficiency_time = df_efficiency_time[df_efficiency_time['episode'].isin(all_episodes)].copy()

# For each episode, calculate the maximum time_saved across all models (baseline)
df_all_models_time = df_combined[df_combined['episode'].isin(all_episodes)].copy()
baseline_time = df_all_models_time.groupby('episode')['total_time_saved'].max().reset_index()
baseline_time.columns = ['episode', 'baseline_time']

# Calculate average time for each model at each episode
df_grouped_efficiency_time = df_filtered_efficiency_time.groupby(['model_display', 'episode'])['total_time_saved'].mean().reset_index()

# Merge with baseline
df_grouped_efficiency_time = df_grouped_efficiency_time.merge(baseline_time, on='episode')

# Calculate efficiency as percentage: (time_saved / baseline) * 100
df_grouped_efficiency_time['efficiency'] = (df_grouped_efficiency_time['total_time_saved'] / df_grouped_efficiency_time['baseline_time']) * 100

# Pivot to get DDQN+mGAT and Heuristic as separate series
df_pivot_time = df_grouped_efficiency_time.pivot(index='episode', columns='model_display', values='efficiency')
...
# Plot efficiency lines for both models
x = df_pivot_time.index.values
y_ddqn = df_pivot_time['DDQN + mGAT'].values
y_heuristic = df_pivot_time['Heuristic'].values
...
# Set labels and title with bold
ax.set_xlabel('Episode', fontweight='bold')
ax.set_ylabel('Time Efficiency (%)', fontweight='bold')
ax.set_title('Time efficiency by models', fontweight='bold')
...
# Save figure
output_path_time_eff = Path("cobotproject/scripts/experiments/rlmodels/mutliagent/two_agent_results/discrete/time_efficiency_by_episode.png")
output_path_time_eff.parent.mkdir(parents=True, exist_ok=True)
plt.savefig(output_path_time_eff, dpi=300, bbox_inches='tight')
print(f"✓ Line chart saved to: {output_path_time_eff}")
plt.close()
