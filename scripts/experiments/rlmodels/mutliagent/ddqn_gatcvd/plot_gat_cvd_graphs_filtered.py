"""
Generate graphs for GAT+CVD test results using FILTERED data
...
# Load filtered combined data
filtered_csv = Path("cobotproject/scripts/experiments/rlmodels/mutliagent/ddqn_gatcvd/gat_cvd_test_results/discrete/filtered_combined_results.csv")

if not filtered_csv.exists():
    print(f"⚠️  Filtered data not found. Please run analyze_gat_cvd_results_filtered.py first.")
    exit()

df_combined = pd.read_csv(filtered_csv)

print(f"✅ Loaded filtered data: {len(df_combined)} episodes")
print(f"   Models: {df_combined['model'].unique()}")
print(f"   Episodes per model: {df_combined.groupby('model').size().to_dict()}")

# Output directory
output_dir = Path("cobotproject/scripts/experiments/rlmodels/mutliagent/ddqn_gatcvd/gat_cvd_test_results/discrete/filtered")
...
# ============================================================================
# BAR GRAPH: Reward by Episode (DDQN+mGAT and 2 other models)
# ============================================================================
...
ax.set_title('Reward by episode (filtered)', fontsize=10)
...
def create_line_chart(df_combined, models_to_plot, model_display_names, metric_column, 
    ...
    # Get all unique episodes (may vary after filtering)
    all_episodes = sorted(df_plot['episode'].unique())
    ...
    if len(all_episodes) > 0:
        ax.set_xlim(min(all_episodes), max(all_episodes))
...
create_line_chart(
    ...
    title='Pick success by models (filtered)',
...
create_line_chart(
    ...
    title='Distance reduced by models (filtered)',
...
create_line_chart(
    ...
    title='Distance efficiency by models (filtered)',
...
create_line_chart(
    ...
    title='Time saved by models (filtered)',
...
create_line_chart(
    ...
    title='Time efficiency by models (filtered)',
    output_filename='time_efficiency_by_episode.png'
)

print("\n✅ All filtered graphs generated successfully!")
