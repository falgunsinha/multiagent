"""
RL Models Experiment - Headless Batch Execution
Modified from franka_rrt_physXLidar_depth_camera_rl_standalone_v2.1.py
Runs multiple RL models in Isaac Sim with real Franka robot and RRT planner
Logs to WandB in real-time and saves results to CSV
"""
...
parser = argparse.ArgumentParser(description="RL Models Experiment - Batch Testing in Isaac Sim")
...
parser.add_argument("--num_cubes", type=int, default=4,
                   help="Number of cubes to spawn (default: 4)")
parser.add_argument("--training_grid_size", type=int, default=3,
                   help="Grid size for cube placement (default: 3x3 grid = 9 positions)")
...
# Import config manager to load experiment config
import yaml
...
if args.experiment not in config_map:
    print(f"[ERROR] Unknown experiment: {args.experiment}")
    print(f"[ERROR] Valid experiments: {list(config_map.keys())}")
    sys.exit(1)

config_path = rlmodels_root / "configs" / config_map[args.experiment]
...
print(f"Models to test: {len(experiment_config.get('models', []))}")
print(f"{'='*60}\n")

# Create SimulationApp BEFORE importing any Isaac Sim modules
from isaacsim import SimulationApp

# GPU Configuration for Isaac Sim
sim_config = {
    "headless": args.headless,
    "active_gpu": 0,  # Use first GPU for rendering
    "physics_gpu": 0,  # Use first GPU for physics simulation
    "multi_gpu": False,  # Disable multi-GPU for consistency
}
...
# Only import UI if not headless
...
try:
    ...
    if torch.cuda.is_available():
        print(f"[CUDA] ✅ CUDA is available!")
        print(f"[CUDA] GPU Device: {torch.cuda.get_device_name(0)}")
        print(f"[CUDA] CUDA Version: {torch.version.cuda}")
        print(f"[CUDA] Number of GPUs: {torch.cuda.device_count()}")
        print(f"[CUDA] Current GPU: {torch.cuda.current_device()}")
        print(f"[CUDA] GPU Memory: {torch.cuda.get_device_properties(0).total_memory / 1024**3:.2f} GB")
    else:
        print("[CUDA] ⚠️  CUDA is NOT available - using CPU for RL models")
        print("[CUDA] This will be significantly slower!")

except ImportError as e:
    print(f"[ERROR] Could not import RL libraries: {e}")
    print("[ERROR] This script requires RL libraries to run")
    simulation_app.close()
    sys.exit(1)

# Import adapters from rlmodels framework
try:
    ...
    print("[Adapters] State/Action adapters loaded successfully")
except ImportError as e:
    print(f"[WARNING] Could not import adapters: {e}")
    print("[WARNING] Adapters will not be available")


class PerformanceTracker:
    """Track and compare performance metrics for different RL methods with WandB logging"""
    ...
    def record_episode(self, episode_reward, episode_steps, episode_time, success_rate):
        """Record episode metrics and log to WandB"""
        self.episode_count += 1

        if self.log_to_wandb:
            wandb.log({
                f"{self.method_name}/episode_reward": episode_reward,
                f"{self.method_name}/episode_steps": episode_steps,
                f"{self.method_name}/episode_time": episode_time,
                f"{self.method_name}/success_rate": success_rate,
...
class FrankaRRTDynamicGrid:
    ...
    def __init__(self, num_cubes=4, training_grid_size=6):
        ...
        # RL model (for batch testing)
        self.rl_model = None
        self.rl_model_path = None
        self.rl_model_type = None  # Will be 'ppo', 'dqn', 'ddqn', 'a2c', 'sac', 'td3'
        self.rl_model_name = None
        self.use_rl = RL_AVAILABLE
        ...
        self.performance_tracker = PerformanceTracker(log_to_wandb=True)

        # Results storage for batch testing
        self.all_results = []
        ...
        # UI elements (only if not headless)
        self.load_btn = None
        self.pick_btn = None
        self.reset_btn = None
        ...
        # Build UI only if not headless
        if not args.headless:
            self.build_ui()

    def build_ui(self):
        """Build UI (only in GUI mode)"""
        if args.headless:
            return
    ...
    def load_rl_model(self, model_path, model_type, model_name):
        """Load an RL model for testing"""
        print(f"\n[Model] Loading: {model_name}")
        print(f"[Model] Path: {model_path}")
        print(f"[Model] Type: {model_type}")

        self.rl_model_path = model_path
        self.rl_model_type = model_type.lower()
        self.rl_model_name = model_name

        try:
            if self.rl_model_type == 'ppo' or self.rl_model_type == 'maskableppo':
                if 'maskable' in self.rl_model_type:
                    self.rl_model = MaskablePPO.load(model_path)
                else:
                    self.rl_model = PPO.load(model_path)
            elif self.rl_model_type == 'dqn':
                self.rl_model = DQN.load(model_path)
            elif self.rl_model_type == 'ddqn':
                # Custom DDQN loader
                self.rl_model = DoubleDQNAgent.load(model_path)
            elif self.rl_model_type == 'a2c':
                self.rl_model = A2C.load(model_path)
            elif self.rl_model_type == 'sac':
                self.rl_model = SAC.load(model_path)
            elif self.rl_model_type == 'td3':
                self.rl_model = TD3.load(model_path)
            else:
                print(f"[ERROR] Unknown model type: {self.rl_model_type}")
                return False
    ...
    def setup_adapters(self, state_adapter_type=None, action_adapter_type=None):
        """Setup state and action adapters"""
        # State adapter
        if state_adapter_type:
            print(f"[Adapter] Setting up state adapter: {state_adapter_type}")
            if state_adapter_type == 'feature_aggregation':
                self.state_adapter = FeatureAggregationAdapter()
            elif state_adapter_type == 'pca':
                self.state_adapter = PCAStateAdapter()
            elif state_adapter_type == 'random_projection':
                self.state_adapter = RandomProjectionAdapter()

        # Action adapter
        if action_adapter_type:
            print(f"[Adapter] Setting up action adapter: {action_adapter_type}")
            if action_adapter_type == 'discrete_mapper':
                self.action_adapter = DiscreteActionMapper()
            elif action_adapter_type == 'continuous_to_discrete':
                self.action_adapter = ContinuousToDiscreteAdapter()
            elif action_adapter_type == 'weighted':
                self.action_adapter = WeightedActionAdapter()
            elif action_adapter_type == 'probabilistic':
                self.action_adapter = ProbabilisticActionAdapter()
    ...
    async def _load_scene(self):
        ...
        try:
            # Get parameters from UI or use existing values (headless mode)
            if not args.headless and self.training_grid_field and self.num_cubes_field:
            ...
            # else: use values from __init__ (already set from args)
                        ...
                        for prim in camera_stage.Traverse():
                            if prim.IsA(UsdGeom.Camera):
                                Sdf.CopySpec(camera_stage.GetRootLayer(), prim.GetPath(),
                                            stage.GetRootLayer(), Sdf.Path("/World/Main_Camera"))
                                break
                    print("[Scene] Camera loaded successfully")
                else:
                    print("[Scene] Warning: Could not load camera USD file")
            except Exception as e:
                print(f"[Scene] Warning: Camera loading failed: {e}")

            # Single update after world setup
            print("[Scene] Updating world...")
            await omni.kit.app.get_app().next_update_async()
            print("[Scene] World updated")

            # Add textured table as robot stand
            print("[Scene] Loading robot stand...")
            ...
            print("[Scene] Robot stand loaded")
            ...
            print("[Scene] Loading Franka robot...")
            ...
            print("[Scene] Franka robot loaded")
            ...
            print("[Scene] Loading Lidar sensor...")
            ...
            # DISABLED IN HEADLESS MODE - too slow to initialize
            if not args.headless:
                ...
                print("[Scene] Depth camera loaded")
            else:
                print("[Scene] Depth camera DISABLED in headless mode (performance)")
            ...
            if not args.headless and self.depth_camera is not None:
            ...
            print("[Scene] Updating after sensors...")
            await omni.kit.app.get_app().next_update_async()
            print("[Scene] Sensors update complete")

            # Add container on table
            print("[Scene] Loading container...")
            container_prim_path = "/World/Container"
            container_usd_path = f"{get_assets_root_path()}/NVIDIA/Assets/DigitalTwin/Assets/Warehouse/Storage/Containers/Container_I/Container_I04_160x120x64cm_PR_V_NVD_01.usd"
            add_reference_to_stage(usd_path=container_usd_path, prim_path=container_prim_path)
            print("[Scene] Container loaded")
            ...
            print("[Scene] Creating visual grid...")
            ...
            print("[Scene] Visual grid created")

            # PyGame-style random placement in FIXED training grid
            print(f"[Scene] Creating {self.num_cubes} cylinders...")
            ...
            print(f"[Scene] {len(self.cubes)} cylinders created")

            # Wait for stage to update before applying shaders
            print("[Scene] Updating stage for shaders...")
            await omni.kit.app.get_app().next_update_async()
            print("[Scene] Stage updated")
            ...
            print("[Scene] Loading grasp configuration...")
            try:
            ...
            print("[Scene] Adding initial obstacles...")
            self._add_initial_obstacles()
            print("[Scene] Initial obstacles added")

            # Initialize physics and reset (batch updates)
            print("[Scene] Initializing physics...")
            self.world.initialize_physics()
            print("[Scene] Physics initialized, resetting world...")
            self.world.reset()
            print("[Scene] World reset complete")

            # Set initial obstacle positions after world reset
            print("[Scene] Setting obstacle positions...")
            ...
            if self.depth_camera is not None:
                print("[Scene] Initializing depth camera...")
                self.depth_camera.initialize(attach_rgb_annotator=False)
                print("[Scene] Depth camera initialized")
            else:
                print("[Scene] Depth camera skipped (disabled in headless mode)")
            ...
            if self.depth_camera is not None:
                ...
                print("[Scene] Depth camera parameters configured")
            else:
                print("[Scene] Depth camera configuration skipped (disabled in headless mode)")
            ...
            if self.depth_camera is not None:
                print("[Scene] Waiting for render variables...")
                for _ in range(5):
                    await omni.kit.app.get_app().next_update_async()
                print("[Scene] Render variables ready")

                # Attach multiple annotators for comprehensive depth data
                print("[Scene] Attaching depth camera annotators...")
                ...
                print("[Scene] Depth camera annotators attached")
            else:
                print("[Scene] Depth camera annotators skipped (disabled in headless mode)")
            ...
            print("[Scene] Robot configured")

            # Wait for robot to settle
            print("[Scene] Waiting for robot to settle...")
            for _ in range(5):
                await omni.kit.app.get_app().next_update_async()
            print("[Scene] Robot settled")

            print("[Scene] Setting up RRT planner...")
            self._setup_rrt()
            print("[Scene] RRT planner setup complete")
...
async def run_batch_experiments():
    """Run batch experiments with multiple models"""
    ...
    wandb.init(
        ...
        config={
            ...
            "grid_size": args.training_grid_size,
            "headless": args.headless,
            **experiment_config
        }
    )
    ...
    print(f"\n{'='*60}")
    print(f"Starting Batch Experiment: {args.experiment}")
    ...
    # Load scene (headless mode - no UI interaction)
    if args.headless:
        print("[Headless] Loading scene...")
        await app._load_scene()
        print("[Headless] Scene loaded successfully!")

    # Get models to test from experiment config
    models_config = experiment_config.get('models', [])

    if not models_config:
        print("[ERROR] No models found in experiment config!")
        wandb.finish()
        simulation_app.close()
        return

    print(f"\n[Batch] Testing {len(models_config)} models")

    # Test each model
    for model_idx, model_config in enumerate(models_config):
        model_name = model_config['name']
        model_path = model_config['path']
        model_type = model_config.get('type', 'ppo')
        num_episodes = model_config.get('episodes', 100)  # Different episodes per model!

        print(f"\n{'='*60}")
        print(f"[{model_idx + 1}/{len(models_config)}] Testing Model: {model_name}")
        print(f"{'='*60}")

        # Load model
        success = app.load_rl_model(model_path, model_type, model_name)
        if not success:
            print(f"[ERROR] Failed to load model {model_name}, skipping...")
            continue

        # Setup adapters if specified
        state_adapter = model_config.get('state_adapter')
        action_adapter = model_config.get('action_adapter')
        app.setup_adapters(state_adapter, action_adapter)

        # Start performance tracking
        app.performance_tracker.start(model_name)

        # Run episodes for this model
        print(f"\n[Episodes] Running {num_episodes} episodes for {model_name}")

        for episode in range(num_episodes):
            episode_start_time = time.time()

            # Reset environment (not async)
            app._on_reset()
            ...
            # Run pick and place with RL model
            # This will use the loaded model for object selection
            app.is_picking = True
            app.current_cube_index = 0
            app.placed_count = 0

            # Run the pick-place loop
            await app._pick_place_loop()

            episode_time = time.time() - episode_start_time

            # Get episode metrics
            summary = app.performance_tracker.get_summary()
            episode_reward = summary.get('successes', 0) * 10 - summary.get('failures', 0) * 1
            episode_steps = len(summary.get('pick_order', []))
            success_rate = summary.get('success_rate', 0)

            # Log to WandB
            app.performance_tracker.record_episode(
                episode_reward, episode_steps, episode_time, success_rate
            )

            # Store results
            app.all_results.append({
                'experiment': args.experiment,
                'model': model_name,
                'episode': episode,
        ...
        # Print model summary
        app.performance_tracker.print_summary()

    # Log summary table to WandB (CSV export handled by experiment_runner.py)
    if app.all_results:
        results_df = pd.DataFrame(app.all_results)
    ...
    print(f"\n{'='*60}")
    print(f"Batch Experiment Completed: {args.experiment}")
    print(f"Total Models Tested: {len(models_config)}")
    print(f"Total Episodes: {len(app.all_results)}")
    print(f"{'='*60}\n")
...
def main():
    ...
    try:
        if args.headless:
            # Run batch experiments in headless mode
            import asyncio
            asyncio.get_event_loop().run_until_complete(run_batch_experiments())
        else:
            # Run with GUI (original behavior)
            app = FrankaRRTDynamicGrid(num_cubes=args.num_cubes, training_grid_size=args.training_grid_size)
