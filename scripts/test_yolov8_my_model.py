"""
Test Custom YOLOv8 Model (augmented_shapes_v2) in Isaac Sim
Simple script to test your custom trained YOLOv8 model on the current viewport.
Uses the custom model trained on 3D geometric shapes (cube, cylinder, container, cuboid).
"""
...
class YOLOv8CustomModelTest:
    """Test custom YOLOv8 model (augmented_shapes_v2) on current Isaac Sim scene"""
    ...
    def build_ui(self):
        """Build simple UI"""
        self.window = ui.Window("Custom YOLOv8 Model Test", width=450, height=350)

        with self.window.frame:
            with ui.VStack(spacing=10):
                ui.Label("Custom YOLOv8 Model Detection Test",
                        alignment=ui.Alignment.CENTER,
                        style={"font_size": 18})

                ui.Spacer(height=10)

                ui.Label("Using: augmented_shapes_v2 (99.4% mAP50)",
                        alignment=ui.Alignment.CENTER,
                        style={"font_size": 12, "color": 0xFF00FF00})

                ui.Label("Classes: cube, cylinder, container, cuboid",
                        alignment=ui.Alignment.CENTER,
    ...
    async def _run_detection_test(self):
        ...
        try:
            ...
            if self.yolo_model is None:
                print("[YOLO] Loading custom model: augmented_shapes_v2...")
                self._update_status("Loading custom model...")
                ...
                try:
                    ...
                    print("[YOLO] Custom model loaded successfully")
                    print("[YOLO] Model: augmented_shapes_v2 (99.4% mAP50)")
                    print("[YOLO] Classes: cube, cylinder, container, cuboid")

                except Exception as e:
                    self._update_status("ERROR: Failed to load custom model!")
            ...
            debug_dir = Path("C:/isaacsim/cobotproject/models/augmented_shapes_v2/test_results")
            ...
            # Run detection with custom YOLOv8 model
            print("[YOLO] Running detection with custom model...")
            self._update_status("Running custom model detection...")

            # Run inference with confidence threshold 0.3 (lower to see more detections)
            print("[YOLO] Using confidence threshold: 0.3")
            results = self.yolo_model(rgb_frame, conf=0.3, verbose=True)
            ...
            if len(detections) == 0:
                ...
                results_text += "Make sure scene contains:\n"
                results_text += "- cube (small cubes)\n"
                results_text += "- cylinder\n"
                results_text += "- container (large cube)\n"
                results_text += "- cuboid (rectangular box)\n"
            else:
                for i, det in enumerate(detections, 1):
            ...
            for det in detections:
                ...
                # Green (0, 255, 0) for high confidence (>0.8)
                # Yellow (255, 255, 0) for medium confidence (0.5-0.8)
                if conf > 0.8:
                    color = (0, 255, 0)  # Green
                    thickness = 2
                else:
...
test_window = YOLOv8CustomModelTest()
