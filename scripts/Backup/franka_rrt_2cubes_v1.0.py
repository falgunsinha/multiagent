"""
Franka RRT Pick and Place - 2 Cuboids
Version for testing with 2 cubes in front of robot
...
class FrankaRRT2Cubes:
    """2-cube pick and place with RRT"""
    ...
    def build_ui(self):
        """Build UI"""
        self.window = ui.Window("Franka RRT - 2 Cubes", width=400, height=300)
        
        with self.window.frame:
            with ui.VStack(spacing=10):
                ui.Label("Franka RRT Pick and Place - 2 Cubes", 
    ...
    async def _load_scene(self):
        ...
        try:
            ...
            # Container position: IN FRONT (positive X), to the right (positive Y = 0.5 to avoid collision)
            container_position = np.array([0.6, 0.5, 0.0])
            ...
            # Add 2 cubes IN FRONT of robot with spacing
            print("=== Adding 2 Cubes ===")
            cube_size = 0.0515
            ...
            print(f"Cube 1 added at: {cube1_position}")
            ...
            self.cube2 = self.world.scene.add(
                DynamicCuboid(
                    ...
                    scale=np.array([cube_size, cube_size, cube_size]),
                    size=1.0,
                    color=np.array([1, 0, 0]),  # Red for second cube
                )
            )
            print(f"Cube 2 added at: {cube2_position}")
    ...
    async def _pick_place_loop(self):
        ...
        try:
            ...
            # Pick cube 1
            if not self.cube1_picked:
                print(">>> Picking Cube 1 (Blue)")
                success = await self._pick_and_place_cube(self.cube1, "Cube 1")
                if success:
                    self.cube1_picked = True
                    self.placed_count += 1
                else:
                    self._update_status("Failed to pick Cube 1")
                    self.is_picking = False
                    return

            # Pick cube 2
            if not self.cube2_picked:
                print(">>> Picking Cube 2 (Red)")
                success = await self._pick_and_place_cube(self.cube2, "Cube 2")
                if success:
                    self.cube2_picked = True
                    self.placed_count += 1
                else:
                    self._update_status("Failed to pick Cube 2")
                    self.is_picking = False
                    return
    ...
    async def _pick_and_place_cube(self, cube, cube_name):
        ...
        try:
            ...
            # Pick position: slightly below cube center to ensure gripper fingers are around cube
            # Cube is 5.15cm tall, center is at 2.575cm from bottom
            # Descend to 1cm from ground (below center) to ensure fingers wrap around cube
            pick_pos = np.array([cube_pos[0], cube_pos[1], 0.01])

            # Place approach: 15cm above container (Y = 0.5)
            container_pos = np.array([0.6, 0.5, 0.0])
            place_height = cube_half + 0.08 + (self.placed_count * cube_size)
            place_approach = container_pos + np.array([0.0, 0.0, place_height + 0.15])

            # Place position: final position
            place_pos = container_pos + np.array([0.0, 0.0, place_height])
            ...
            # Step 10: Return to home position (safe configuration for next pick)
            ...
            # Step 11: Close gripper to reset for next pick
            print("Step 11: Closing gripper (reset for next pick)")
            articulation_controller = self.franka.get_articulation_controller()
            close_action = ArticulationAction(
                joint_positions=self.gripper.joint_closed_positions,
                joint_indices=np.array([7, 8])
...
# Create and show UI
print("\n" + "="*60)
print("Franka RRT - 2 Cubes Pick and Place")
print("="*60 + "\n")
