...
class FrankaRRT4Cubes:
    ...
    async def _pick_and_place_cube(self, cube, cube_name):
        ...
        try:
            ...
            # Calculate neighboring positions for 4 cubes
            # Cube 1: CENTER
            # Cube 2: RIGHT of center (+Y direction)
            # Cube 3: FRONT of center (+X direction)
            # Cube 4: FRONT-RIGHT diagonal

            # Spacing between neighboring cubes (cube is 5.15cm, use 7cm spacing)
            neighbor_spacing = 0.07  # 7cm

            # Define offsets for each cube position
            # Pattern:  [2]
            #         [1][4]
            #           [3]
            placement_offsets = [
                np.array([0.0, 0.0, 0.0]),              # Cube 1: CENTER
                np.array([0.0, neighbor_spacing, 0.0]),  # Cube 2: RIGHT
                np.array([neighbor_spacing, 0.0, 0.0]),  # Cube 3: FRONT
                np.array([neighbor_spacing, neighbor_spacing, 0.0])  # Cube 4: FRONT-RIGHT
            ]

            # Get offset for current cube
            offset = placement_offsets[self.placed_count]
            ...
            # Debug output
            position_names = ["CENTER", "RIGHT", "FRONT", "FRONT-RIGHT"]
            print(f"  → Placing at {position_names[self.placed_count]} position")
            ...
            # Step 13: Close gripper to reset for next pick
            print("Step 13: Closing gripper (reset for next pick)")
            articulation_controller = self.franka.get_articulation_controller()
            close_action = ArticulationAction(
                joint_positions=self.gripper.joint_closed_positions,
                joint_indices=np.array([7, 8])
