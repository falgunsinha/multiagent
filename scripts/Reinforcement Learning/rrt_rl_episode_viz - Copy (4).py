...
class RRTRLEpisodeVisualizer:
    ...
    def __init__(self, grid_size: int, num_cubes: int, model_path: Optional[str],
        ...
        # Load model first to get max_objects from model if provided
        self.model = None
        model_max_objects = None
        if model_path and os.path.exists(model_path):
            logger.info(f"Loading RL model from: {model_path}")
            # Load model without env to inspect observation space
            import zipfile
            import json
            try:
                with zipfile.ZipFile(model_path, 'r') as archive:
                    ...
                    if 'observation_space' in data:
                        ...
                        if isinstance(obs_space, dict):
                            if 'shape' in obs_space:
                                ...
                                logger.warning(f"observation_space keys: {obs_space.keys()}")
                                raise KeyError("Cannot find shape in observation_space")
                        else:
                            # observation_space might be a tuple/list
                    ...
                    logger.info(f"Model observation space: {obs_space_shape} dims = {model_max_objects} objects × 6 features")
            except Exception as e:
                logger.warning(f"Could not extract observation space from model: {e}")
                logger.info(f"Using default max_objects={num_cubes}")

        # Create environment with correct max_objects
        max_objects = model_max_objects if model_max_objects else num_cubes
        logger.info(f"Creating environment with max_objects={max_objects}, num_cubes={num_cubes}")
        ...
        self.wrapped_env = wrapped_env  # Keep reference to wrapped env for step/reset

        # Now load model with correct env
        if model_path and os.path.exists(model_path):
            self.model = MaskablePPO.load(model_path, env=vec_env)
            logger.info(f"Model loaded successfully!")
        else:
            logger.info("No model provided - using greedy baseline")
    ...
    def generate_new_episode(self):
        ...
        # Log environment state after reset
        logger.info(f"After reset: total_objects={self.env.total_objects}, num_cubes={self.env.num_cubes}, "
                   f"object_positions length={len(self.env.object_positions)}")

        # Log cube positions to check for overlaps
        ee_grid_x, ee_grid_y = self.env.rrt_estimator._world_to_grid(self.env.ee_position[:2])
        logger.info(f"EE grid position: ({ee_grid_x}, {ee_grid_y})")
        for i in range(self.env.total_objects):
            pos = self.env.object_positions[i]
            grid_x, grid_y = self.env.rrt_estimator._world_to_grid(pos[:2])
            logger.info(f"  Cube {i}: world=({pos[0]:.3f}, {pos[1]:.3f}), grid=({grid_x}, {grid_y})")
        ...
        while not done and step < self.env.max_steps:
            ...
            if self.model is not None:
                ...
                # Note: obs is already in the correct format from vec_env
                action, _ = self.model.predict(obs, action_masks=action_mask, deterministic=True)
                action = int(action)
            else:
                action = self._greedy_action()

            cube_pos = self.env.object_positions[action][:2]
            num_picked = len(self.env.objects_picked)
            num_unpicked = self.env.total_objects - num_picked
            logger.info(f"  Step {step+1}: Selected cube {action} at position ({cube_pos[0]:.3f}, {cube_pos[1]:.3f}) "
                       f"[Picked: {num_picked}/{self.env.total_objects}, Unpicked: {num_unpicked}]")
            ...
            if path is None or len(path) == 0:
                logger.warning(f"    RRT FAILED: No path found! Euclidean dist={dist_to_ee:.3f}m")
                # Log obstacle configuration for debugging
                logger.debug(f"      Obstacles: {len(self.static_obstacles)} static, {num_unpicked-1} unpicked cubes")
            else:
                logger.info(f"    RRT SUCCESS: Path length={path_length:.3f}m, waypoints={len(path)}, clearance={path_clearance:.3f}")
                # Check for obstacle collisions
                collision_detected = self._check_path_obstacle_collision(path)
                if collision_detected:
                    logger.error(f"    COLLISION DETECTED: Path crosses obstacle!")
        ...
        # Log episode summary
        successful_picks = sum(1 for path in episode.paths if path and len(path) > 0)
        failed_picks = len(episode.paths) - successful_picks
        logger.info(f"\nEpisode Summary:")
        logger.info(f"  Total selections: {len(episode.actions)}")
        logger.info(f"  Successful paths: {successful_picks}")
        logger.info(f"  Failed paths: {failed_picks}")
        logger.info(f"  Total reward: {episode.metrics['total_reward']:.2f}")
    ...
    def _add_random_obstacles(self, num_obstacles: int):
        ...
        if len(empty_cells) >= num_obstacles:
            ...
            logger.info(f"  Added {num_obstacles} obstacles at grid positions: {self.static_obstacles}")
    ...
    def _update_rrt_grid_for_planning(self, target_cube_idx: int):
        ...
        # Log obstacle configuration
        logger.debug(f"      Planning for cube {target_cube_idx}: {len(obstacle_positions)} static obstacles, "
                    f"{len(unpicked_cube_positions)} unpicked cubes as obstacles")
    ...
    def _update_rrt_grid_dynamic(self, target_cube_idx: int, picked_cubes: set):
        """
        Update RRT occupancy grid for dynamic replanning during animation.

        Obstacles include:
        1. Static obstacles (randomly placed)
        2. Unpicked cubes (EXCEPT the target cube and already picked cubes)

        Args:
            target_cube_idx: Index of the cube we're planning to pick (not treated as obstacle)
            picked_cubes: Set of cube indices that have already been picked during animation
        """
        # Get static obstacle positions
        obstacle_positions = []
        for grid_x, grid_y in self.static_obstacles:
        ...
        # Get unpicked cube positions (excluding target cube and already picked cubes)
        unpicked_cube_positions = []
        for i in range(self.env.total_objects):
            if i not in picked_cubes and i != target_cube_idx:
                unpicked_cube_positions.append(self.env.object_positions[i])

        # Log obstacle configuration
        logger.debug(f"      [REPLAN] Planning for cube {target_cube_idx}: {len(obstacle_positions)} static obstacles, "
                    f"{len(unpicked_cube_positions)} unpicked cubes as obstacles, {len(picked_cubes)} already picked")
    ...
    def _get_rrt_path(self, cube_idx: int, skip_grid_update: bool = False) -> Optional[List[Tuple[int, int]]]:
        """
        ...
        if cube_idx >= self.env.total_objects:
            logger.warning(f"      Invalid cube_idx {cube_idx} >= total_objects {self.env.total_objects}")
            return None
        ...
        logger.debug(f"      RRT planning: start_grid={ee_grid}, goal_grid={goal_grid}")

        # Check if start == goal
        if ee_grid == goal_grid:
            logger.debug(f"      Start == Goal, returning empty path")
        ...
        if rx is None or ry is None:
            logger.debug(f"      RRT planning returned None (no path found)")
            return []  # No path found
        else:
            path = [(rx[i], ry[i]) for i in range(len(rx))]
            logger.debug(f"      RRT planning succeeded with {len(path)} waypoints")
            return path
    ...
    def _check_path_obstacle_collision(self, path: List[Tuple[int, int]]) -> bool:
        """
        ...
        # Define collision threshold (in grid cells)
        # If path waypoint is within this distance of obstacle center, it's a collision
        collision_threshold = 0.5  # grid cells

        for path_grid_x, path_grid_y in path:
            for obs_grid_x, obs_grid_y in self.static_obstacles:
                # Calculate grid distance
                grid_dist = np.sqrt((path_grid_x - obs_grid_x)**2 + (path_grid_y - obs_grid_y)**2)

                if grid_dist < collision_threshold:
                    logger.error(f"      Collision: path waypoint ({path_grid_x}, {path_grid_y}) "
                               f"too close to obstacle ({obs_grid_x}, {obs_grid_y}), dist={grid_dist:.2f} cells")
                    return True
    ...
    def calculate_reward_components(self, path_length, dist_to_container, obstacle_proximity,
        ...
        # 4. Reachability penalty (0.0 or -5.0)
        # reachability flag: 1.0 = reachable, 0.0 = unreachable
        reachability_penalty = 0.0 if reachability >= 0.5 else -5.0
    ...
    def draw_info_panel(self, x, y, width, height):
        ...
        # Performance Metrics (Rewards) Box
        box_y = self.draw_box(x, current_y, width, 160, "Performance Metrics (Rewards)")
        ...
        reward_items = [
            ...
            ("Reachability reward:", f"{self.current_reward_components['r_reachability']:.3f}"),
    ...
    def update_playback(self):
        ...
        if self.animation_phase == 0:
            ...
            if self.phase_timer >= self.playback_speed:  # Wait just 1 frame
                ...
                if self.episode_step < len(self.current_episode.actions):
                    ...
                    # DYNAMIC RRT REPLANNING: Plan path with current obstacle state
                    # Update RRT grid to reflect cubes already picked
                    picked_cubes = set(self.current_episode.picked_cubes[:self.episode_step])
                    self._update_rrt_grid_dynamic(self.selected_cube_idx, picked_cubes)

                    # Replan RRT path from current state (skip grid update since we just did it)
                    replanned_path = self._get_rrt_path(self.selected_cube_idx, skip_grid_update=True)

                    # Use replanned path instead of stored path
                    self.current_path = replanned_path

                    # Recalculate metrics with replanned path
                    path_length = self._calculate_path_length(replanned_path) if replanned_path else 999.0
                    obstacle_proximity = self._calculate_obstacle_proximity(self.selected_cube_idx)
                    reachability = replanned_path is not None
                    path_clearance = self._calculate_path_clearance(replanned_path) if replanned_path else 0.0
                    dist_to_ee = np.linalg.norm(self.env.object_positions[self.selected_cube_idx][:2] - self.env.ee_position[:2])
                    dist_to_container = np.linalg.norm(self.env.object_positions[self.selected_cube_idx][:2] - self.env.container_position[:2])

                    # Update observation components with replanned metrics
                    self.current_obs_components['dist_to_ee'] = dist_to_ee
                    self.current_obs_components['dist_to_container'] = dist_to_container
                    self.current_obs_components['obstacle_proximity'] = obstacle_proximity
                    self.current_obs_components['reachability'] = reachability
                    self.current_obs_components['path_clearance'] = path_clearance
                    ...
                    # Log animation start with replanning status
                    if replanned_path:
                        logger.info(f"  [ANIMATION] Starting cube {self.selected_cube_idx}: RRT REPLANNED - {len(replanned_path)} waypoints, {path_length:.3f}m")
                    else:
                        logger.warning(f"  [ANIMATION] Starting cube {self.selected_cube_idx}: RRT REPLAN FAILED - no path found")
        ...
        # Phase 1: Show selected cube (blue border only)
            ...
            if self.phase_timer >= self.selection_delay:
                ...
                if has_valid_path:
                    logger.info(f"  [ANIMATION] Phase 1->2: Showing path with {len(self.current_path)} waypoints")
                    ...
                    logger.warning(f"  [ANIMATION] Phase 1->3: SKIPPING path animation (no valid path)")
        ...
        # Phase 2: Path animation (path grows towards target)
            ...
            if self.phase_timer >= self.path_animation_duration:
                logger.info(f"  [ANIMATION] Phase 2->3: Path animation complete")
        ...
        # Phase 3: Cube + path disappear together
            ...
            if self.phase_timer >= self.post_animation_delay:
                logger.info(f"  [ANIMATION] Phase 3->0: Cube picked, moving to next")
