...
import gc
import os as os_module  # Rename to avoid conflict with omni.isaac.core.utils.stage.os
...
class FrankaRRTTrainer:
    ...
    def _create_random_obstacles(self):
        ...
        for idx, (grid_x, grid_y) in enumerate(selected_cells):
            ...
            self.obstacles.append(obstacle)  # Track obstacle for re-randomization
    ...
    def randomize_cube_positions(self):
        ...
        print(f"[TRAINER] Randomized positions for {len(self.cubes)} cubes (with shuffled assignment)")
...
def main():
    ...
    if args.use_wandb:
        try:
            ...
            wandb.init(
                ...
                name=f"rrt_isaacsim_grid{args.grid_size}_cubes{args.num_cubes}",
                config={
                    "method": "rrt_isaacsim",
            ...
            print("Please check your W&B login: C:\\isaacsim\\python.bat -m wandb login")
            sys.exit(1)
    ...
    env = ObjectSelectionEnvRRT(
        ...
        num_cubes=args.num_cubes,
        training_grid_size=args.grid_size,
        execute_picks=args.execute_picks,  # Use command-line argument
