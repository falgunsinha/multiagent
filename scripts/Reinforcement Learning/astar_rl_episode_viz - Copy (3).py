...
class AStarRLEpisodeVisualizer:
    ...
    def draw_info_panel(self, x, y, width, height):
        ...
        # Header
        header_surf = self.font_header.render("A* Motion Planner", True, HEADER_COLOR)
    ...
    def create_astar_graph_surface(self, width, height):
        """Create A* path planning visualization with explored nodes (PythonRobotics style)"""
        ...
        # Simulate explored nodes visualization (cyan filled area around path)
        # This creates a visual effect similar to A* exploration
        if self.animation_phase == 2 and self.path_progress > 0:
            ...
            # Create explored region (scatter plot with cyan dots)
            explored_x = []
            explored_y = []
            for i in range(num_points):
                px, py = path_x[i], path_y[i]
                # Add points in a small radius around each path point to simulate exploration
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        ex, ey = px + dx, py + dy
                        ...
                        if 0 <= ex < self.grid_size and 0 <= ey < self.grid_size:
                            ...
                            if (ex, ey) not in self.static_obstacles:
                                explored_x.append(ex)
                                explored_y.append(ey)

            # Plot explored nodes (cyan dots with transparency)
            if explored_x:
                ax.plot(explored_x, explored_y, ".", color='cyan', markersize=8, alpha=0.4)
        ...
        # Draw animated portion of path (red line - PythonRobotics style)
        num_points = max(2, int(len(self.current_path) * self.path_progress))
        ...
        ax.set_title('A* Path Planning', color='#64C8FF', fontsize=9, fontweight='bold')
        ax.tick_params(colors='white', labelsize=6)
