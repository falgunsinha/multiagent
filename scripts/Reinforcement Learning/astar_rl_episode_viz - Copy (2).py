...
class AStarRLEpisodeVisualizer:
    ...
    def draw_info_panel(self, x, y, width, height):
        ...
        # Observation Metrics Box
        box_y = self.draw_box(x, current_y, width, 160, "Observation Metrics")

        obs_items = [
            ("Distance to EE:", f"{self.current_obs_components['dist_to_ee']:.3f}"),
            ("Distance to container:", f"{self.current_obs_components['dist_to_container']:.3f}"),
            ("Obstacle proximity:", f"{self.current_obs_components['obstacle_proximity']:.3f}"),
            ("Reachability:", f"{self.current_obs_components['reachability']:.3f}"),
            ("Path clearance:", f"{self.current_obs_components['path_clearance']:.3f}"),
            ("Invalid pick:", f"{self.current_obs_components['invalid_pick']:.3f}"),
        ]
