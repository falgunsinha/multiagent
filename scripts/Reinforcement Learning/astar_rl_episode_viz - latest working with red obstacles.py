...
class AStarRLEpisodeVisualizer:
    ...
    def generate_new_episode(self):
        ...
        print(f"[EPISODE] Obstacles: {len(self.static_obstacles)}")
        ...
        print(f"[EPISODE] #{self.episode_count}: {len(episode.actions)} picks, reward: {episode.metrics['total_reward']:.2f}")
    ...
    def draw_path(self, cell_size, grid_x, grid_y, grid_h, agent_pos):
        ...
        if len(points) >= 2:
            ...
            if len(curved_points) >= 2:
                ...
                pygame.draw.circle(self.screen, (0, 255, 0), curved_points[0], 10)  # Green at start
                pygame.draw.circle(self.screen, (255, 0, 0), curved_points[-1], 10)  # Red at end
    ...
    def update_playback(self):
        ...
        if self.animation_phase == 0:
            ...
            if self.phase_timer >= self.playback_speed:  # Wait just 1 frame
                ...
                if self.episode_step < len(self.current_episode.actions):
                    ...
                    # Log animation start
                    print(f"\n[ANIM] Step {self.episode_step + 1}/{len(self.current_episode.actions)}: Cube {self.selected_cube_idx}")
                    ...
                    # Episode complete
                    print("\n[EPISODE] Complete! Generating new episode...")
                    self.generate_new_episode()
        ...
        # Phase 1: Show selected cube (blue border only)
            ...
            if self.phase_timer >= self.selection_delay:
                ...
                if has_valid_path:
                    print(f"[ANIM]   Phase 1→2: Selection complete, showing path")
                    # Move to phase 2: Path animation
                    self.animation_phase = 2
                    self.phase_timer = 0
                else:
                    print(f"[ANIM]   Phase 1→3: No path (cube at EE), skipping to disappear")
        ...
        # Phase 2: Path animation
            ...
            if self.phase_timer >= self.path_animation_duration:
                print(f"[ANIM]   Phase 2→3: Path complete, starting disappear")
        ...
        # Phase 3: Cube + path disappear together
            ...
            if self.phase_timer >= self.post_animation_delay:
                print(f"[ANIM]   Phase 3→0: Cube removed, ready for next")
