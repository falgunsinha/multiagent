"""
Train MAPPO Two-Agent System with RRT Visualization

Trains:
- Agent 1 (DDQN): Pre-trained, frozen
- Agent 2 (MAPPO): Learns reshuffling policy

Environment: RRT path planning with visualization
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train MAPPO two-agent system with RRT visualization")
...
def main():
    ...
    # Create base environment (RRT visualization)
    print("Creating RRT environment...")
    ...
    model_name = f"mappo_rrt_viz_grid{args.grid_size}_cubes{args.num_cubes}_{timestamp}"
    wandb_run_name = f"rrt_viz_grid{args.grid_size}_cubes{args.num_cubes}_{timestamp}"

    wandb_logger = WandBLogger(
        project_name=args.wandb_project,
        run_name=args.run_name or wandb_run_name,
        config=vars(args),
        tags=["mappo", "rrt_viz", f"grid{args.grid_size}", f"cubes{args.num_cubes}"],
    )
    ...
    # Training loop (same as A* version)
    episode = 0
    total_steps = 0
    ...
    while total_steps < args.timesteps:
        ...
        while not done and buffer.ptr < args.buffer_size:
            ...
            buffer.add(obs=obs, action=action, reward=reward, value=value, log_prob=log_prob, done=done, action_mask=action_mask)
            ...
            for epoch in range(args.ppo_epochs):
                ...
                for start in range(0, args.buffer_size, args.batch_size):
                    ...
                    value_loss, policy_loss, entropy, ratio = trainer.ppo_update(
                        obs_batch=data['observations'][mb_indices],
                        actions_batch=data['actions'][mb_indices],
                        value_preds_batch=data['values'][mb_indices],
                        return_batch=data['returns'][mb_indices],
                        old_action_log_probs_batch=data['log_probs'][mb_indices],
                        adv_targ=data['advantages'][mb_indices],
                        action_mask_batch=data['action_masks'][mb_indices],
                    )
