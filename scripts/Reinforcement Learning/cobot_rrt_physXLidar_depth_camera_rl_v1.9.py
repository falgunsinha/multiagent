"""
Franka RRT Pick and Place - Dynamic Grid with Kinematics Solver + RL Object Selection
...
NEW in v2.0:
- RL-based object selection (optional via --rl_model argument)
- Greedy baseline (pick closest first) as default
- Random object placement within grid cells for better generalization

Usage:
    # With RL model
    C:\isaacsim\python.bat franka_rrt_physXLidar_depth_camera_rl_v1.9.py --rl_model "models/object_selection/object_selection_20251128_122313_final.zip"

    # Without RL (greedy baseline)
    C:\isaacsim\python.bat franka_rrt_physXLidar_depth_camera_rl_v1.9.py
"""
...
parser = argparse.ArgumentParser(description="Franka RRT Pick-and-Place with RL Object Selection")
parser.add_argument("--rl_model", type=str, default=None,
                   help="Path to trained RL model (e.g., models/object_selection/ppo_final.zip)")
...
class FrankaRRTDynamicGrid:
    """Dynamic grid pick and place with RRT + RL object selection"""
    ...
    async def _load_scene(self):
        ...
        try:
            ...
            # Create visual grid on ground plane
            create_visual_grid(start_x, start_y, grid_extent_x, grid_extent_y, cube_spacing, self.grid_length, self.grid_width)

            # Random placement parameters
            random_offset_range = 0.03  # ±3cm random offset within each grid cell
            print(f"[GRID] Random placement enabled: ±{random_offset_range*100:.0f}cm offset per cell")
            ...
            for row in range(self.grid_length):
                for col in range(self.grid_width):
                    ...
                    # Add random offset within cell (±3cm)
                    ...
                    cube_x = base_x + random_offset_x
                    cube_y = base_y + random_offset_y
    ...
    def _load_rl_model(self):
        """Load trained RL model for object selection"""
        if not self.use_rl or not self.rl_model_path:
            return

        try:
            print(f"\n[RL] Loading model from: {self.rl_model_path}")

            # Create dummy environment for loading
            max_objects = self.grid_length * self.grid_width
            ...
            if Path(vecnorm_path).exists():
                print(f"[RL] Loading VecNormalize from: {vecnorm_path}")
                vec_env = VecNormalize.load(vecnorm_path, vec_env)
                vec_env.training = False
                vec_env.norm_reward = False

            # Load model
            self.rl_model = PPO.load(self.rl_model_path, env=vec_env)
    ...
    def _get_rl_pick_order(self):
        ...
        if self.use_rl and self.rl_model is not None:
            try:
                ...
                print(f"[RL] Greedy pick order: {pick_order}")
            except Exception as e:
        ...
        try:
            ...
            for order_idx in range(self.current_cube_index, total_cubes):
                try:
                    ...
                    self.current_cube_index = order_idx
